...
class SliceView(QtWidgets.QWidget):
    ...
    def _slice_mask(self, m):
        ...
        if self.view_type == "coronal" and self.is_mip_active:
            try:
                out = np.max(m, axis=1)
                return out[::-1, :] if _BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY else out
            except Exception:
                return None
        if self.view_type == "axial":
            return m[self.slice_idx][::-1]
        if self.view_type == "coronal":
            out = m[:, self.slice_idx, :]
            return out[::-1, :] if _BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY else out
        out = m[:, :, self.slice_idx]
        return out[::-1, :] if _BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY else out
    ...
    def _get_slice_arrays(self):
        ...
        if self.view_type == "coronal" and self.is_mip_active:
            ...
            if _BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY:
                return ct_slice[::-1, :], pet_mip[::-1, :], mask_mip[::-1, :]
            return ct_slice, pet_mip, mask_mip
        if self.view_type == "axial":
            return ct[self.slice_idx][::-1], pet[self.slice_idx][::-1], m[self.slice_idx][::-1]
        if self.view_type == "coronal":
            ct2, pet2, m2 = ct[:, self.slice_idx, :], pet[:, self.slice_idx, :], m[:, self.slice_idx, :]
            if _BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY:
                return ct2[::-1, :], pet2[::-1, :], m2[::-1, :]
            return ct2, pet2, m2
        ct2, pet2, m2 = ct[:, :, self.slice_idx], pet[:, :, self.slice_idx], m[:, :, self.slice_idx]
        if _BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY:
            return ct2[::-1, :], pet2[::-1, :], m2[::-1, :]
    ...
    def display_to_voxel(self, x_disp, y_disp):
        ...
        if self.view_type == "axial":
            return self.slice_idx, np.clip((ny-1)-yi, 0, ny-1), np.clip(xi, 0, nx-1)
        if self.view_type == "coronal":
            z = (nz - 1) - yi if _BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY else yi
            return np.clip(z, 0, nz-1), self.slice_idx, np.clip(xi, 0, nx-1)
        z = (nz - 1) - yi if _BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY else yi
        return np.clip(z, 0, nz-1), np.clip(xi, 0, ny-1), self.slice_idx
    def voxel_to_display(self, z, y, x):
        ...
        if self.view_type == "coronal":
            z_disp = (nz - 1 - z) if _BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY else z
            return float(x) * sx, float(z_disp) * sz
        z_disp = (nz - 1 - z) if _BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY else z
        return float(y) * sy, float(z_disp) * sz
    ...
    def lasso_end(self, x, y):
        ...
        if self.view_type == "axial":
            self.mask[z, :, :] |= _polyfill_2d(self.mask.shape[1], self.mask.shape[2], xs, ys)[::-1]
        elif self.view_type == "coronal":
            poly = _polyfill_2d(self.mask.shape[0], self.mask.shape[2], xs, ys)
            self.mask[:, y, :] |= (poly[::-1, :] if _BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY else poly)
        else:
            poly = _polyfill_2d(self.mask.shape[0], self.mask.shape[1], xs, ys)
            self.mask[:, :, x] |= (poly[::-1, :] if _BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY else poly)
...
# Fibroglandular tissue on contrast/non-contrast CT is usually soft-tissue
# attenuation but must stay below obvious pectoralis/vessel/calcification range.
# Keeping a dedicated FG window prevents posterior muscle bands being painted as FG.
_BD_HU_FG_MIN = -115
_BD_HU_FG_MAX = 170
# v36 strict FGT extraction window; keep configurable for CT protocol tuning.
_BD_FG_STRICT_MIN_HU = -45.0
_BD_FG_STRICT_MAX_HU = 150.0
# v42: automatic peritumoral exclusion is available but OFF by default.
# The stable workflow uses manual tumour/peritumoral/pectoralis ROIs and never
# runs heavy inference from the manual Segment FG button.
_BD_ENABLE_AUTO_PERITUMORAL_EXCLUSION = False
_BD_PERITUMORAL_HALO_MM = 20.0
_BD_PERITUMORAL_SKIN_SHELL_MM = 10.0
_BD_PERITUMORAL_SOFT_MIN_HU = -110.0
_BD_PERITUMORAL_SOFT_MAX_HU = 180.0
_BD_HU_MUSCLE_MIN = 25
_BD_HU_MUSCLE_MAX = 125
_BD_HU_SKIN_MIN = -220
_BD_HU_SKIN_MAX = 220
_BD_HU_VESSEL_MIN = 95
_BD_HU_VESSEL_MAX = 399
_BD_HU_CALC_MIN = 180
_BD_HU_CLIP_MIN = 400
_BD_CLIP_MAX_VOXELS = 1200
_BD_VESSEL_MAX_VOXELS = 2500
# Main viewer orientation must remain identical to the original PET/CT viewer.
# Do NOT flip SliceView coronal/sagittal planes here; changing this affects live
# viewing, crosshair mapping, lasso/paint coordinates, and ROI editing.
_BD_FLIP_CORONAL_SAGITTAL_FOR_DISPLAY = False

# Breast segmentation figure export can apply an export-only vertical flip for
# coronal/sagittal panels without altering the live viewer orientation.
_BD_FLIP_BREAST_EXPORT_CORONAL_SAGITTAL = True

BREAST_SEGMENTATION_STRICT_VERSION = "v46_fg_visible_export_fix_2026_04_26"
# v27 safety: PET-guided auto tumour exclusion was the main UI-freeze culprit
# because it dilated/labelled the entire PET/CT volume before the worker thread
# started.  Keep it OFF by default; use the manually painted tumour exclusion ROI.
_BD_ENABLE_PET_AUTO_TUMOR_EXCLUSION = False
# If manual breast ROIs are labelled, Compute Density will use the fast manual
# ROI pathway and will not invoke TotalSegmentator/autosegmentation.
_BD_PREFER_MANUAL_ROI_DENSITY_WHEN_AVAILABLE = True
# v29 display/segmentation sensitivity knobs.  These are deliberately
# conservative-speed operations: no PET whole-volume component labelling.
# Increase _BD_FG_VISUAL_THICKEN_ITER to 2 only if FG strands still appear too faint.
_BD_FG_VISUAL_THICKEN_ITER = 1
_BD_FG_DISPLAY_THICKEN_ITER = 1
_BD_SKIN_SHELL_EROSION_ITER = 1
_BD_PECTORALIS_POSTERIOR_EXTENSION_VOXELS = 52
...
def _bd_side_from_mask(mask: np.ndarray) -> str | None:
    """Infer breast side from the X-centroid of a side mask."""
    ...
    if x.size == 0:
        return None
    return "right" if float(x.mean()) < (nx / 2.0) else "left"


def _bd_lateral_side_mask(shape_zyx, side: str | None, gap_frac: float = 0.035) -> np.ndarray:
    """Boolean mask for the lateral breast half, excluding the central mediastinum.

    The main failure in difficult PET/CT breast cases is that HU thresholding
    accepts mediastinal vessels/heart/sternum as "breast-like" soft tissue.
    A small midline gap prevents those central structures being re-labelled as
    pectoralis or fibroglandular tissue while preserving the true breast mound.
    """
    nz, ny, nx = [int(v) for v in shape_zyx]
    mid = nx // 2
    gap = max(6, int(round(nx * float(gap_frac))))
    out = np.zeros((nz, ny, nx), dtype=bool)
    if side == "right":
        out[:, :, :max(1, mid - gap)] = True
    elif side == "left":
        out[:, :, min(nx - 1, mid + gap):] = True
    else:
        out[:, :, :max(1, mid - gap)] = True
        out[:, :, min(nx - 1, mid + gap):] = True
    return out
...
def _bd_keep_largest_anterior_component(mask: np.ndarray, ny: int,
                                         anterior_y_fraction: float = 0.68,
                                         side: str | None = None) -> np.ndarray:
    """Keep breast-mound components and reject mediastinal/posterior fragments.

    v25 refinement: rather than taking one global largest component, this keeps
    sizeable lateral anterior components on the requested side and discards
    central mediastinal/vascular components.  This is important when the tumour
    is exophytic or when contrast-enhanced vessels have HU similar to glandular
    tissue.
    """
    if mask is None or not np.any(mask):
        return np.zeros_like(mask, dtype=bool) if mask is not None else mask

    mask = mask.astype(bool) & _bd_lateral_side_mask(mask.shape, side)
    if not np.any(mask):
    ...
    nx = int(mask.shape[2])
    mid = nx / 2.0
    central_gap = max(6, int(round(nx * 0.035)))
    y_threshold = ny * float(anterior_y_fraction)

    keep_labels: list[int] = []
    min_keep = max(500, int(max_size * 0.08))
    for lbl in range(1, n + 1):
        size = int(sizes[lbl]) if lbl < len(sizes) else 0
        if size < min_keep:
            continue
        comp = labeled == lbl
        ...
        if centroid_y >= y_threshold:
            continue
        if side == "right" and centroid_x >= (mid - central_gap):
            continue
        if side == "left" and centroid_x <= (mid + central_gap):
            continue
    ...
    return np.isin(labeled, keep_labels).astype(bool)

def _bd_fill_holes_slicewise(mask: np.ndarray) -> np.ndarray:
    """Fill small closed holes slice-by-slice without bridging distant 3-D anatomy."""
    if mask is None or not np.any(mask):
        return np.zeros_like(mask, dtype=bool) if mask is not None else mask
    out = np.zeros_like(mask, dtype=bool)
    try:
        for z in range(mask.shape[0]):
            out[z] = _bd_ndi.binary_fill_holes(mask[z]).astype(bool)
        return out
    except Exception:
        try:
            return _bd_ndi.binary_fill_holes(mask).astype(bool)
        except Exception:
            return mask.astype(bool)


def _bd_morph_close_open(mask: np.ndarray, close_iter: int = 1, open_iter: int = 0) -> np.ndarray:
    """Small safe morphology helper for 3-D Boolean masks."""
    if mask is None:
        return mask
    ...
    try:
        if close_iter:
            out = _bd_ndi.binary_closing(out, structure=st, iterations=int(close_iter))
        if open_iter:
            out = _bd_ndi.binary_opening(out, structure=st, iterations=int(open_iter))
    except Exception:
        pass
    return out.astype(bool)


def _bd_mask_bbox(mask: np.ndarray, pad_zyx=(0, 0, 0)):
    """Return clipped bounding-box slices for a 3-D mask, or None if empty."""
    if mask is None or not np.any(mask):
        return None
    z, y, x = np.nonzero(mask)
    nz, ny, nx = mask.shape
    pz, py, px = [int(v) for v in pad_zyx]
    return (
        slice(max(0, int(z.min()) - pz), min(nz, int(z.max()) + pz + 1)),
        slice(max(0, int(y.min()) - py), min(ny, int(y.max()) + py + 1)),
        slice(max(0, int(x.min()) - px), min(nx, int(x.max()) + px + 1)),
    )


def _bd_component_filter(mask: np.ndarray, min_size: int = 1, max_size: Optional[int] = None,
                         predicate=None) -> np.ndarray:
    """Label a Boolean mask and keep components matching size/predicate criteria."""
    out = np.zeros_like(mask, dtype=bool)
    if mask is None or not np.any(mask):
        return out
    labels, n_labels = _bd_ndi.label(mask.astype(bool))
    if n_labels == 0:
        return out
    objects = _bd_ndi.find_objects(labels)
    ...
    for lbl in range(1, n_labels + 1):
        ...
        if sl is None:
            continue
        comp_local = labels[sl] == lbl
        if predicate is not None and not predicate(lbl, sl, comp_local, size):
            continue
        out[sl] |= comp_local
    return out
...
# TotalSegmentator 2.12.0 task-specific labels used by this module.
# - body task: skin
# - breasts task: breast
# - abdominal_muscles task: pectoralis_major_right / pectoralis_major_left
# - thigh_shoulder_muscles task: pectoralis_minor (combined label; side is split spatially)
_BD_TS_BREAST_TASK = "breasts"
_BD_TS_BREAST_STRUCTURES = ["breast"]
_BD_TS_BODY_TASK = "body"
_BD_TS_BODY_SKIN_STRUCTURES = ["skin"]
_BD_TS_PECTORALIS_MAJOR_TASK = "abdominal_muscles"
_BD_TS_PECTORALIS_MAJOR_BY_SIDE = {
    "right": ["pectoralis_major_right"],
    "left":  ["pectoralis_major_left"],
}
_BD_TS_PECTORALIS_MINOR_TASK = "thigh_shoulder_muscles"
_BD_TS_PECTORALIS_MINOR_STRUCTURES = ["pectoralis_minor"]
_BD_TS_HAS_FIBROGLANDULAR_CLASS = False  # TotalSegmentator 2.12.0 has no FGT-specific class

# v33 safety: do not run TotalSegmentator inside the manual "Segment
# Fibroglandular Tissue" button. That button must stay responsive and use the
# already-painted breast/tumour ROIs. Use Compute Density (Auto) if full
# TotalSegmentator-assisted automatic breast segmentation is desired.
_BD_USE_TS_DURING_MANUAL_FG = False
# Pectoralis minor subtask is slow on many systems and is not essential for
# subtracting the posterior chest-wall pectoralis band; pectoralis major is used
# when TotalSegmentator mode is explicitly enabled.
_BD_TS_INCLUDE_PECTORALIS_MINOR = False
...
class BreastSegmentor:
    """Robust CT breast-density segmentation for difficult PET/CT breast cases.

    v26 strict refinement:
    - Pectoralis is deliberately confined to a *lateral, posterior chest-wall*
      band and is never allowed in the central mediastinum.
    - Fibroglandular tissue is confined to the lateral breast mound and is
      removed from the posterior pectoral/chest-wall band.
    - Small true FG islands are preserved, especially in a sparse contralateral
      breast.
    """

    _ANTERIOR_FRACTION = 0.58
    _CENTRAL_GAP_FRAC = 0.095

    def __init__(self, ct_sitk=None, use_total_segmentator: bool = True):
        self._ct_sitk_ref = ct_sitk
        self._use_total_segmentator = bool(use_total_segmentator)
        self._ts_cache: dict = {}
        self._last_seg_backend = "TotalSegmentator-enabled + v33" if self._use_total_segmentator else "manual-fast-no-TS + v33"

    def _set_ct_reference(self, ct_sitk) -> None:
        if ct_sitk is not self._ct_sitk_ref:
            self._ct_sitk_ref = ct_sitk
            self._ts_cache = {}

    def _ts_fetch_masks(self, task: str, structures: list[str], fast: bool = False) -> dict:
        """Run TotalSegmentator on a small ROI subset and cache the masks."""
        structures = [str(s) for s in (structures or []) if s]
        if (not self._use_total_segmentator) or not structures or not _BD_TS_AVAILABLE or self._ct_sitk_ref is None:
            return {}
        key = (str(task), bool(fast), tuple(sorted(structures)))
        if key in self._ts_cache:
            return self._ts_cache[key]
        out: dict = {}
        try:
            nib_img = _bd_sitk_to_nib(self._ct_sitk_ref)
            nz, ny, nx = sitk.GetArrayFromImage(self._ct_sitk_ref).shape
            with tempfile.TemporaryDirectory(prefix="bd_ts_roi_") as tmpdir:
                out_dir = os.path.join(tmpdir, "segs")
                _bd_ts_api(
                    nib_img, out_dir,
                    task=str(task),
                    roi_subset=list(dict.fromkeys(structures)),
                    fast=bool(fast),
                    verbose=False,
                    statistics=False,
                    radiomics=False,
                ...
                for s in structures:
                    ...
                    if os.path.exists(p):
                        seg_xyz = _bd_nib.load(p).get_fdata() > 0.5
                        out[s] = seg_xyz.transpose(2, 1, 0).astype(bool)
                    else:
                        out[s] = np.zeros((nz, ny, nx), dtype=bool)
        except Exception:
            out = {}
        self._ts_cache[key] = out
        return out

    def _ts_skin_shell_mask(self, breast_mask: np.ndarray) -> np.ndarray:
        if breast_mask is None or not np.any(breast_mask):
            return np.zeros_like(breast_mask, dtype=bool)
        masks = self._ts_fetch_masks(task=_BD_TS_BODY_TASK, structures=_BD_TS_BODY_SKIN_STRUCTURES, fast=True)
        ts_skin = masks.get("skin")
        if ts_skin is None or not np.any(ts_skin):
            return np.zeros_like(breast_mask, dtype=bool)
        core = self._side_core(breast_mask, erode_iter=0)
        if not np.any(core):
            core = breast_mask.astype(bool)
        try:
            local = _bd_ndi.binary_dilation(core.astype(bool), structure=np.ones((1, 3, 3), dtype=bool), iterations=1)
        except Exception:
            local = core.astype(bool)
        skin = ts_skin.astype(bool) & local
        try:
            skin = _bd_remove_small_objects(skin.astype(bool), min_size=4)
        except Exception:
            pass
        return skin.astype(bool)

    def _ts_breast_mask(self, ct_volume: np.ndarray) -> np.ndarray:
        """Return the TotalSegmentator 2.12.0 breasts-task mask if available."""
        masks = self._ts_fetch_masks(task=_BD_TS_BREAST_TASK, structures=_BD_TS_BREAST_STRUCTURES, fast=True)
        breast = masks.get("breast") if masks else None
        if breast is None or not np.any(breast):
            return np.zeros_like(ct_volume, dtype=bool)
        try:
            breast = _bd_ndi.binary_closing(breast.astype(bool), structure=np.ones((1, 3, 3), dtype=bool), iterations=1)
            breast = _bd_fill_holes_slicewise(breast.astype(bool))
            breast = _bd_remove_small_objects(breast.astype(bool), min_size=200)
        except Exception:
            pass
        return breast.astype(bool)


    def _ts_pectoralis_mask(self, ct_volume: np.ndarray, breast_mask: np.ndarray) -> np.ndarray:
        """Extract TotalSegmentator pectoralis labels with strict HU/lateral constraints.

        v36: do not replace the pectoralis with geometric rectangles. Use the exact
        TotalSegmentator pectoralis major/minor masks, then restrict them to the
        correct lateral side and soft-tissue HU range.
        """
        if breast_mask is None or not np.any(breast_mask):
            return np.zeros_like(ct_volume, dtype=bool)
        side = _bd_side_from_mask(breast_mask)
        if side not in ("right", "left"):
            return np.zeros_like(ct_volume, dtype=bool)

        major = self._ts_fetch_masks(
            task=_BD_TS_PECTORALIS_MAJOR_TASK,
            structures=_BD_TS_PECTORALIS_MAJOR_BY_SIDE[side],
            fast=True,
        )
        minor = {}
        if globals().get("_BD_TS_INCLUDE_PECTORALIS_MINOR", False):
            minor = self._ts_fetch_masks(
                task=_BD_TS_PECTORALIS_MINOR_TASK,
                structures=_BD_TS_PECTORALIS_MINOR_STRUCTURES,
                fast=True,
            )

        union = np.zeros_like(ct_volume, dtype=bool)
        for m in list(major.values()) + list(minor.values()):
            if m is not None and np.any(m) and m.shape == union.shape:
                union |= m.astype(bool)
        if not np.any(union):
            return union

        lateral = _bd_lateral_side_mask(
            ct_volume.shape, side,
            gap_frac=max(self._CENTRAL_GAP_FRAC, 0.12)
        )

        pec = (union.astype(bool) &
               lateral.astype(bool) &
               (ct_volume >= -10.0) &
               (ct_volume <= 150.0))

        try:
            pec = _bd_ndi.binary_closing(
                pec.astype(bool),
                structure=np.ones((1, 3, 5), dtype=bool),
                iterations=1
            )
            pec = _bd_remove_small_objects(pec.astype(bool), min_size=8)
        except Exception:
            pass
        return pec.astype(bool)

    def _breast_roi_mask(self, ct_volume: np.ndarray,
                         ct_sitk=None) -> np.ndarray:
        """Build a 3-D Boolean mask restricting analysis to lateral anterior thorax."""
        ...
        if "z_top" in ts and "z_diaphragm" in ts:
            z_top = int(ts["z_top"])
            z_diaph = int(ts["z_diaphragm"])
            self._last_seg_backend = "TotalSegmentator + v26-strict"
        else:
            try:
                z_top, z_diaph = _bd_detect_thorax_z_range(ct_volume)
            except Exception:
                z_top, z_diaph = 0, nz // 2
            self._last_seg_backend = "HU-heuristic + v26-strict"

        if "y_posterior_vertebrae" in ts:
            y_end = int(ts["y_posterior_vertebrae"])
        else:
            y_end = max(1, int(ny * self._ANTERIOR_FRACTION))
        y_end = int(np.clip(y_end, 1, ny - 1))
        ...
        roi[max(0, z_top):min(nz, z_diaph + 1), :y_end, :] = True
        roi &= body
        # Critical: remove central mediastinum/sternum corridor before any
        # tissue-layer labelling.
        roi &= _bd_lateral_side_mask(ct_volume.shape, None, gap_frac=self._CENTRAL_GAP_FRAC)
        return roi.astype(bool)

    def _side_core(self, breast_mask: np.ndarray, erode_iter: int = 0) -> np.ndarray:
        """Return lateral breast core with the central mediastinum removed."""
        if breast_mask is None or not np.any(breast_mask):
            return np.zeros_like(breast_mask, dtype=bool)
        side = _bd_side_from_mask(breast_mask)
        core = breast_mask.astype(bool) & _bd_lateral_side_mask(
            breast_mask.shape, side, gap_frac=self._CENTRAL_GAP_FRAC
        )
        if erode_iter > 0 and np.any(core):
            try:
                core = _bd_ndi.binary_erosion(
                    core, structure=np.ones((1, 3, 3), dtype=bool),
                    iterations=int(erode_iter)
                )
            except Exception:
                pass
        return core.astype(bool)

    def _skin_shell_mask(self, ct_volume: np.ndarray, breast_mask: np.ndarray) -> np.ndarray:
        """Robust thin skin shell for both breasts.

        v31: if TotalSegmentator body-skin is available, prefer it for a more
        anatomically faithful olive-green breast skin shell on both sides.
        Otherwise fall back to the slice-wise geometric/CT heuristic.
        """
        if breast_mask is None or not np.any(breast_mask):
            return np.zeros_like(ct_volume, dtype=bool)
        ts_skin = self._ts_skin_shell_mask(breast_mask)
        if ts_skin is not None and np.any(ts_skin):
            return ts_skin.astype(bool)
        core = self._side_core(breast_mask, erode_iter=0)
        if not np.any(core):
            core = breast_mask.astype(bool)

        shell = np.zeros_like(core, dtype=bool)
        try:
            st2 = np.ones((3, 3), dtype=bool)
            for z in range(core.shape[0]):
                sl = core[z].astype(bool)
                if not np.any(sl):
                    continue
                try:
                    sl_fill = _bd_ndi.binary_fill_holes(sl)
                except Exception:
                    sl_fill = sl
                er = _bd_ndi.binary_erosion(sl_fill, structure=st2,
                                            iterations=int(_BD_SKIN_SHELL_EROSION_ITER),
                                            border_value=0)
                rim = sl_fill & ~er
                shell[z] = rim & _bd_ndi.binary_dilation(sl, structure=st2, iterations=1)
        except Exception:
            try:
                inner = _bd_ndi.binary_erosion(core, structure=np.ones((1, 3, 3), dtype=bool), iterations=1)
                shell = core & ~inner
            except Exception:
                shell = core.copy()

        hu_skin = (ct_volume >= _BD_HU_SKIN_MIN) & (ct_volume <= _BD_HU_SKIN_MAX)
        skin = shell & hu_skin
        if np.count_nonzero(skin) < max(20, int(0.12 * np.count_nonzero(shell))):
            skin = shell
        try:
            skin = _bd_ndi.binary_dilation(skin.astype(bool), structure=np.ones((1, 2, 2), dtype=bool), iterations=1)
            skin &= core
        except Exception:
            pass
        try:
            skin = _bd_remove_small_objects(skin.astype(bool), min_size=6)
        except Exception:
            pass
        return skin.astype(bool)

    def _posterior_band(self, breast_mask: np.ndarray, start_frac: float = 0.70,
                        extra_voxels: int = 2) -> np.ndarray:
        """Posterior band within a lateral breast-side mask."""
        out = np.zeros_like(breast_mask, dtype=bool)
        if breast_mask is None or not np.any(breast_mask):
            return out
        core = self._side_core(breast_mask, erode_iter=0)
        if not np.any(core):
            return out
        y_coords = np.where(core.any(axis=(0, 2)))[0]
        if y_coords.size < 2:
            return out
        y_ant, y_post = int(y_coords.min()), int(y_coords.max())
        y_span = max(1, y_post - y_ant)
        y_start = int(y_ant + float(start_frac) * y_span)
        y_stop = min(core.shape[1], y_post + int(extra_voxels) + 1)
        out[:, max(0, y_start):y_stop, :] = True
        return (out & core).astype(bool)

    def _middle_breast_core(self, breast_mask: np.ndarray, ap_low: float = 0.18,
                            ap_high: float = 0.76, x_trim_frac: float = 0.06) -> np.ndarray:
        """Central breast parenchymal zone, away from superficial skin and deep chest wall."""
        out = np.zeros_like(breast_mask, dtype=bool)
        if breast_mask is None or not np.any(breast_mask):
            return out
        core = self._side_core(breast_mask, erode_iter=0)
        ...
        if not np.any(core):
            return out
        zc, yc, xc = np.nonzero(core)
        if len(yc) < 3:
            return core.astype(bool)
        y_ant, y_post = int(yc.min()), int(yc.max())
        x_min, x_max = int(xc.min()), int(xc.max())
        y_span = max(1, y_post - y_ant)
        x_span = max(1, x_max - x_min)
        y0 = int(round(y_ant + float(ap_low) * y_span))
        y1 = int(round(y_ant + float(ap_high) * y_span))
        x0 = int(round(x_min + float(x_trim_frac) * x_span))
        x1 = int(round(x_max - float(x_trim_frac) * x_span))
        out[:, max(0, y0):min(core.shape[1], y1 + 1), max(0, x0):min(core.shape[2], x1 + 1)] = True
        out &= core
        try:
            out = _bd_ndi.binary_erosion(out.astype(bool), structure=np.ones((1, 3, 3), dtype=bool), iterations=1)
        except Exception:
            pass
        if np.count_nonzero(out) < max(25, int(0.10 * np.count_nonzero(core))):
            out = core.astype(bool)
        return out.astype(bool)
    def _pectoralis_mask(self, ct_volume: np.ndarray, breast_mask: np.ndarray) -> np.ndarray:
        """Estimate pectoralis as a posterior chest-wall muscle slab.

        v35: prefer TotalSegmentator pectoralis major/minor masks and expand them into
        a continuous posterior chest-wall muscle slab. Fall back to the heuristic
        detector only if TS labels are unavailable.
        """
        out = np.zeros_like(ct_volume, dtype=bool)
        if breast_mask is None or not np.any(breast_mask):
            return out
        ...
        if len(yc) < 3:
            return out
        z_min, z_max = int(zc.min()), int(zc.max())
        y_ant, y_post = int(yc.min()), int(yc.max())
        x_min, x_max = int(xc.min()), int(xc.max())
        y_span = max(1, y_post - y_ant)

        y_start = max(0, int(y_ant + 0.54 * y_span))
        y_stop = min(ny, y_post + int(_BD_PECTORALIS_POSTERIOR_EXTENSION_VOXELS) + 10)
        zsl = slice(max(0, z_min - 6), min(nz, z_max + 8))
        pad_x = 32
        if side == "right":
            xsl = slice(max(0, x_min - pad_x), min(nx, min(x_max + pad_x + 1, mid - 2)))
        elif side == "left":
            xsl = slice(max(0, max(x_min - pad_x, mid + 2)), min(nx, x_max + pad_x + 1))
        else:
            xsl = slice(max(0, x_min - pad_x), min(nx, x_max + pad_x + 1))

        search = np.zeros_like(ct_volume, dtype=bool)
        search[zsl, slice(y_start, y_stop), xsl] = True
        lateral = _bd_lateral_side_mask(ct_volume.shape, side, gap_frac=max(self._CENTRAL_GAP_FRAC, 0.12))
        search &= lateral
        allowed_core_overlap = self._posterior_band(core, start_frac=0.82, extra_voxels=6)
        search &= (~core | allowed_core_overlap)

        ts_pec = self._ts_pectoralis_mask(ct_volume, breast_mask)
        if ts_pec is not None and np.any(ts_pec):
            pec = ts_pec.astype(bool) & search
            hu_soft = (ct_volume >= -35.0) & (ct_volume <= 160.0) & search
            try:
                seed = _bd_ndi.binary_dilation(pec.astype(bool), structure=np.ones((1, 3, 5), dtype=bool), iterations=2)
                support = hu_soft & _bd_ndi.binary_dilation(seed.astype(bool), structure=np.ones((1, 5, 7), dtype=bool), iterations=1)
                pec = pec | support
                pec = _bd_ndi.binary_closing(pec.astype(bool), structure=np.ones((1, 5, 7), dtype=bool), iterations=1)
                pec = _bd_remove_small_objects(pec.astype(bool), min_size=24)
            except Exception:
                pass
            return (pec & search).astype(bool)

        cand = (search &
                (ct_volume >= max(5.0, _BD_HU_MUSCLE_MIN - 15.0)) &
                (ct_volume <= min(150.0, _BD_HU_MUSCLE_MAX + 35.0)) &
                (ct_volume < _BD_HU_CALC_MIN))

        if np.any(cand):
            try:
                cand = _bd_ndi.binary_closing(cand.astype(bool), structure=np.ones((1, 4, 7), dtype=bool), iterations=1)
                cand = _bd_ndi.binary_opening(cand.astype(bool), structure=np.ones((1, 2, 2), dtype=bool), iterations=1)
            except Exception:
                pass

        central_gap = max(10, int(nx * max(self._CENTRAL_GAP_FRAC, 0.12)))

        def _is_pectoral(lbl, sl, comp_local, size):
            if size < 24 or size > 120000:
                return False
            zz, yy, xx = np.nonzero(comp_local)
            if yy.size == 0:
                return False
            cy = float(yy.mean() + sl[1].start)
            cx = float(xx.mean() + sl[2].start)
            if abs(cx - mid) < central_gap:
                return False
            if side == "right" and cx > mid - central_gap:
                return False
            if side == "left" and cx < mid + central_gap:
                return False
            if cy < (y_ant + 0.54 * y_span):
                return False
            vals = ct_volume[sl][comp_local]
            if vals.size == 0:
                return False
            mean_hu = float(np.mean(vals))
            p90 = float(np.percentile(vals, 90))
            if mean_hu < 0 or p90 > 160:
                return False
            dims = [max(1, sl[i].stop - sl[i].start) for i in range(3)]
            z_span_c, y_span_c, x_span_c = dims
            fill = float(size / max(1, np.prod(dims)))
            return (x_span_c >= 4 and y_span_c <= 90 and fill <= 0.98)

        pec = np.zeros_like(ct_volume, dtype=bool)
        if np.any(cand):
            try:
                pec = _bd_component_filter(cand.astype(bool), min_size=24, max_size=120000,
                                           predicate=_is_pectoral).astype(bool)
            except Exception:
                pec = np.zeros_like(ct_volume, dtype=bool)

        min_required = max(20, int(0.01 * np.count_nonzero(core)))
        if np.count_nonzero(pec) < min_required:
            soft = (search &
                    (ct_volume >= -10.0) &
                    (ct_volume <= min(140.0, _BD_HU_MUSCLE_MAX + 25.0)) &
                    (ct_volume < _BD_HU_CALC_MIN))
            fallback = np.zeros_like(ct_volume, dtype=bool)
            for z in range(max(0, z_min - 2), min(nz, z_max + 3)):
                core_sl = core[z]
                if not np.any(core_sl):
                    continue
                x_cols = np.where(core_sl.any(axis=0))[0]
                y_rows = np.where(core_sl.any(axis=1))[0]
                if x_cols.size < 2 or y_rows.size < 2:
                    continue
                x0 = max(0, int(x_cols.min()) - 10)
                x1 = min(nx, int(x_cols.max()) + 11)
                y_post_sl = int(y_rows.max())
                y0 = max(0, y_post_sl - 2)
                y1 = min(ny, y_post_sl + 28)
                local = soft[z, y0:y1, x0:x1]
                if not np.any(local):
                    continue
                try:
                    lab, nlab = _bd_ndi.label(local)
                except Exception:
                    nlab = 0
                    lab = None
                if nlab <= 0:
                    continue
                keep_local = np.zeros_like(local, dtype=bool)
                best_score = None
                best_lbl = None
                for lbl in range(1, int(nlab) + 1):
                    ...
                    if yy.size == 0:
                        continue
                    mean_y = float(yy.mean() + y0)
                    mean_x = float(xx.mean() + x0)
                    if side == "right" and mean_x >= mid - 2:
                        continue
                    if side == "left" and mean_x <= mid + 2:
                        continue
                    if mean_y < y_post_sl - 1:
                        continue
                    vals = ct_volume[z, y0:y1, x0:x1][comp]
                    if vals.size == 0:
                        continue
                    score = (size, mean_y, float(np.mean(vals)))
                    if best_score is None or score > best_score:
                        best_score = score
                        best_lbl = lbl
                if best_lbl is not None:
                    keep_local |= (lab == best_lbl)
                    fallback[z, y0:y1, x0:x1] |= keep_local
            try:
                fallback = _bd_ndi.binary_closing(fallback.astype(bool), structure=np.ones((1, 2, 3), dtype=bool), iterations=1)
                fallback = _bd_remove_small_objects(fallback.astype(bool), min_size=10)
            except Exception:
                pass
            pec = fallback if np.count_nonzero(fallback) > np.count_nonzero(pec) else pec

        pec &= search
        try:
            pec = _bd_ndi.binary_closing(pec.astype(bool), structure=np.ones((1, 5, 7), dtype=bool), iterations=1)
            pec = _bd_remove_small_objects(pec.astype(bool), min_size=10)
        except Exception:
            pass
        return pec.astype(bool)
    def _clips_calcification_mask(self, ct_volume: np.ndarray, breast_mask: np.ndarray) -> np.ndarray:
        """Detect small high-HU clips/calcifications inside the lateral breast ROI."""
        if breast_mask is None or not np.any(breast_mask):
            return np.zeros_like(ct_volume, dtype=bool)
        lateral = _bd_lateral_side_mask(
            ct_volume.shape, _bd_side_from_mask(breast_mask),
            gap_frac=self._CENTRAL_GAP_FRAC
        )
        cand = (((ct_volume >= _BD_HU_CLIP_MIN) |
                 ((ct_volume >= _BD_HU_CALC_MIN) & (ct_volume < _BD_HU_CLIP_MIN))) &
                breast_mask.astype(bool) & lateral)

        def _clip_like(lbl, sl, comp_local, size):
            vals = ct_volume[sl][comp_local]
            if vals.size == 0:
                return False
            max_hu = float(np.max(vals))
            mean_hu = float(np.mean(vals))
            dims = [sl[i].stop - sl[i].start for i in range(3)]
            if max_hu >= _BD_HU_CLIP_MIN:
                return size <= _BD_CLIP_MAX_VOXELS and max(dims) <= 30
            return size <= 160 and mean_hu >= _BD_HU_CALC_MIN and max(dims) <= 12

        return _bd_component_filter(cand, min_size=1,
                                    max_size=max(_BD_CLIP_MAX_VOXELS, 160),
                                    predicate=_clip_like).astype(bool)

    def _vessel_mask(self, ct_volume: np.ndarray, breast_mask: np.ndarray,
                     skin: np.ndarray | None = None,
                     pectoralis: np.ndarray | None = None,
                     clips: np.ndarray | None = None) -> np.ndarray:
        """Detect only small/tubular lateral enhancing components."""
        if breast_mask is None or not np.any(breast_mask):
            return np.zeros_like(ct_volume, dtype=bool)
        occupied = np.zeros_like(ct_volume, dtype=bool)
        for m in (skin, pectoralis, clips):
            if m is not None:
        ...
        cand = ((ct_volume >= _BD_HU_VESSEL_MIN) &
                (ct_volume <= _BD_HU_VESSEL_MAX) &
                breast_mask.astype(bool) & lateral & ~occupied)

        def _vessel_like(lbl, sl, comp_local, size):
            if size < 8 or size > min(_BD_VESSEL_MAX_VOXELS, 2500):
                return False
            vals = ct_volume[sl][comp_local]
            if vals.size == 0:
                return False
            ...
            dims = [max(1, sl[i].stop - sl[i].start) for i in range(3)]
            longest = max(dims)
            shortest = max(1, min(dims))
            ratio = float(longest / shortest)
            fill = float(size / max(1, np.prod(dims)))
            return (max_hu >= 120 and mean_hu >= 85 and
                    ratio >= 2.2 and fill <= 0.45 and shortest <= 8)

        out = _bd_component_filter(cand, min_size=8,
                                   max_size=min(_BD_VESSEL_MAX_VOXELS, 2500),
                                   predicate=_vessel_like)
        try:
            out = _bd_remove_small_objects(out.astype(bool), min_size=8)
        except Exception:
            pass
        return out.astype(bool)
    ...
    def segment_whole_breast(self, ct_volume: np.ndarray,
                             ...
                             ct_sitk=None) -> dict:
        """Segment right and left whole-breast regions as Boolean 3-D arrays."""
        self._set_ct_reference(ct_sitk)
        nz, ny, nx = ct_volume.shape
        ts_breast = np.zeros_like(ct_volume, dtype=bool)
        if roi_mask is None:
            # Prefer the TotalSegmentator 2.12.0 breasts task for automatic whole-breast
            # segmentation. If it is absent/fails, fall back to the anterior-thorax ROI.
            ts_breast = self._ts_breast_mask(ct_volume)
            if np.any(ts_breast):
                roi_mask = ts_breast.astype(bool)
                self._last_seg_backend = "TotalSegmentator 2.12.0 breasts + tissue cleanup"
            else:
                roi_mask = self._breast_roi_mask(ct_volume, ct_sitk=ct_sitk)

        breast_hu = ((ct_volume >= _BD_HU_BREAST_MIN) &
                     (ct_volume <= _BD_HU_BREAST_MAX))
        # If the breasts task is available, trust its anatomical extent and only
        # lightly constrain by HU. Otherwise use the older HU-based ROI.
        if np.any(ts_breast):
            breast = roi_mask.astype(bool) & ((ct_volume >= -350) & (ct_volume <= 300))
        else:
            breast = breast_hu & roi_mask.astype(bool)
        breast &= _bd_lateral_side_mask(ct_volume.shape, None, gap_frac=self._CENTRAL_GAP_FRAC)
        breast = _bd_morph_close_open(breast, close_iter=1, open_iter=0)
        breast = _bd_fill_holes_slicewise(breast)
        try:
            breast = _bd_remove_small_objects(breast.astype(bool), min_size=900)
        except Exception:
            pass
        ...
        midline = nx // 2
        gap = max(10, int(round(nx * self._CENTRAL_GAP_FRAC)))
        right_mask = np.zeros_like(breast, dtype=bool)
        left_mask = np.zeros_like(breast, dtype=bool)
        right_mask[:, :, :max(1, midline - gap)] = breast[:, :, :max(1, midline - gap)]
        left_mask[:, :, min(nx - 1, midline + gap):] = breast[:, :, min(nx - 1, midline + gap):]

        def _refine_side(side_mask: np.ndarray, side: str) -> np.ndarray:
            if side_mask is None or not np.any(side_mask):
                return np.zeros_like(breast, dtype=bool)
            side_mask = _bd_keep_largest_anterior_component(
                side_mask.astype(bool), ny, anterior_y_fraction=0.64, side=side
            )
            side_mask = _bd_fill_holes_slicewise(side_mask)
            side_mask = _bd_morph_close_open(side_mask, close_iter=1, open_iter=0)
            pec = self._pectoralis_mask(ct_volume, side_mask)
            if pec.any():
                side_mask = side_mask & ~pec
            try:
                side_mask = _bd_remove_small_objects(side_mask.astype(bool), min_size=500)
            except Exception:
                pass
            return side_mask.astype(bool)

        return {"right_mask": _refine_side(right_mask, "right"),
                "left_mask": _refine_side(left_mask, "left")}

    def _adaptive_fg_threshold(self, ct_volume: np.ndarray, core_mask: np.ndarray) -> float:
        """Return a robust HU threshold for fibroglandular tissue inside a painted breast ROI.

        Manual PET/CT breast ROIs vary substantially in CT noise, contrast phase, and
        lesion burden. A fixed -35 HU cutoff was too strict and erased sparse true
        fibroglandular strands. This helper estimates the fat/soft-tissue split
        from the ROI itself and then constrains it to a safe clinical range.
        """
        vals = ct_volume[core_mask.astype(bool)]
        vals = vals[np.isfinite(vals)]
        vals = vals[(vals >= -180.0) & (vals <= 180.0)]
        if vals.size < 50:
            return -82.0
        thr = None
        try:
            from skimage.filters import threshold_otsu as _bd_threshold_otsu
            thr = float(_bd_threshold_otsu(vals.astype(np.float32)))
        except Exception:
            try:
                hist, edges = np.histogram(vals, bins=96, range=(-180.0, 180.0))
                centers = (edges[:-1] + edges[1:]) / 2.0
                region = (centers >= -95.0) & (centers <= 20.0)
                if np.any(region):
                    idxs = np.where(region)[0]
                    thr = float(centers[idxs[np.argmin(hist[idxs])]])
            except Exception:
                thr = None
        if thr is None or not np.isfinite(thr):
            thr = -55.0
        # Bias threshold downward so sparse glandular strands remain visible.
        return float(np.clip(float(thr) - 18.0, -105.0, -42.0))

    def _rebalance_fg_threshold(self, ct_volume: np.ndarray, candidate_core: np.ndarray,
                                base_thr: float) -> float:
        """Sanity-check the FG threshold so the whole breast is not re-labelled as FG.

        The v29 code accidentally used ``min(thr, _BD_HU_FG_MIN)``, which pushed the
        lower HU bound too low and could colour almost the entire breast ROI as
        fibroglandular.  This helper tightens or relaxes the threshold based on the
        fraction of the candidate core selected.
        """
        if candidate_core is None or not np.any(candidate_core):
            return float(base_thr)
        thr = float(np.clip(base_thr, _BD_HU_FG_MIN, 30.0))
        total = int(np.count_nonzero(candidate_core))
        if total <= 0:
            return thr

        def _frac(t: float) -> float:
            m = (ct_volume >= float(t)) & (ct_volume <= _BD_HU_FG_MAX) & candidate_core
            return float(np.count_nonzero(m)) / float(total)

        frac = _frac(thr)
        if frac > 0.72:
            # Too much of the breast is being labelled FG -> tighten aggressively.
            for dt in (8.0, 15.0, 25.0, 35.0, 45.0, 55.0):
                t2 = min(45.0, thr + dt)
                if _frac(t2) <= 0.60:
                    thr = t2
                    frac = _frac(thr)
                ...
                thr = min(55.0, thr + 60.0)
                frac = _frac(thr)
        elif frac < 0.01:
            # If the mask is almost empty, relax slightly so sparse genuine strands survive.
            thr = max(_BD_HU_FG_MIN, thr - 8.0)
        return float(thr)


    def _voxels_from_mm(self, mm: float, axis: int = 1) -> int:
        """Convert a physical distance to integer voxels for the current CT spacing."""
        mm = float(mm)
        if mm <= 0:
            return 0
        spacing = None
        try:
            if self._ct_sitk_ref is not None:
                sx, sy, sz = self._ct_sitk_ref.GetSpacing()
        ...
        if spacing is None or len(spacing) != 3:
            spacing = (1.0, 1.0, 1.0)
        try:
            step = max(1e-3, float(spacing[int(axis)]))
        except Exception:
            step = 1.0
        return max(1, int(np.ceil(mm / step)))

    def build_peritumoral_exclusion(self, ct_volume: np.ndarray, breast_mask: np.ndarray,
                                    tumor_mask: np.ndarray | None, details: dict | None = None) -> np.ndarray:
        """Create an automatic peritumoural exclusion halo within the breast ROI.

        v41 no-freeze change:
        The earlier version used a large full-volume 3-D binary dilation. On a
        full PET/CT volume this can take several minutes. This version crops to
        the local tumour/breast bounding box first, then performs the same logic
        only inside that small box. It is still optional and is disabled by the
        checkbox unless the user explicitly enables it.
        """
        out = np.zeros_like(ct_volume, dtype=bool)
        if (not globals().get("_BD_ENABLE_AUTO_PERITUMORAL_EXCLUSION", True) or
                breast_mask is None or not np.any(breast_mask) or
                tumor_mask is None or not np.any(tumor_mask)):
            return out

        core = self._side_core(breast_mask.astype(bool), erode_iter=0)
        if not np.any(core):
            core = breast_mask.astype(bool)
        if not np.any(core):
            return out

        tumor_in_side = tumor_mask.astype(bool) & core
        if not np.any(tumor_in_side):
            try:
                lateral = _bd_lateral_side_mask(ct_volume.shape, _bd_side_from_mask(core), gap_frac=self._CENTRAL_GAP_FRAC)
                tumor_in_side = tumor_mask.astype(bool) & lateral
            except Exception:
                tumor_in_side = tumor_mask.astype(bool)
        if not np.any(tumor_in_side):
            return out

        halo_y = self._voxels_from_mm(float(globals().get("_BD_PERITUMORAL_HALO_MM", 20.0)), axis=1)
        halo_x = self._voxels_from_mm(float(globals().get("_BD_PERITUMORAL_HALO_MM", 20.0)), axis=2)
        halo_z = max(1, self._voxels_from_mm(float(globals().get("_BD_PERITUMORAL_HALO_MM", 20.0)) * 0.35, axis=0))

        # Work on a cropped ROI around the tumour + halo margin to avoid full-volume
        # dilation freezes.
        try:
            zz, yy, xx = np.nonzero(tumor_in_side)
            z0 = max(0, int(zz.min()) - halo_z - 2)
            z1 = min(ct_volume.shape[0], int(zz.max()) + halo_z + 3)
            y0 = max(0, int(yy.min()) - halo_y - 2)
            y1 = min(ct_volume.shape[1], int(yy.max()) + halo_y + 3)
            x0 = max(0, int(xx.min()) - halo_x - 2)
            x1 = min(ct_volume.shape[2], int(xx.max()) + halo_x + 3)
            sl = (slice(z0, z1), slice(y0, y1), slice(x0, x1))
        except Exception:
            sl = (slice(None), slice(None), slice(None))

        tumor_l = tumor_in_side[sl]
        core_l = core[sl]
        ct_l = ct_volume[sl]
        soft_l = ((ct_l >= float(globals().get("_BD_PERITUMORAL_SOFT_MIN_HU", -110.0))) &
                  (ct_l <= float(globals().get("_BD_PERITUMORAL_SOFT_MAX_HU", 180.0))))

        try:
            halo_l = _bd_ndi.binary_dilation(
                tumor_l.astype(bool),
                structure=np.ones((1 + 2 * halo_z, 1 + 2 * halo_y, 1 + 2 * halo_x), dtype=bool),
                iterations=1,
            )
        except Exception:
            halo_l = tumor_l.astype(bool)

        exclusion_l = halo_l & core_l & soft_l

        skin = details.get("skin") if isinstance(details, dict) else None
        if skin is not None and np.any(skin):
            skin_l = skin.astype(bool)[sl]
            skin_y = self._voxels_from_mm(float(globals().get("_BD_PERITUMORAL_SKIN_SHELL_MM", 10.0)), axis=1)
            skin_x = self._voxels_from_mm(float(globals().get("_BD_PERITUMORAL_SKIN_SHELL_MM", 10.0)), axis=2)
            try:
                skin_halo_l = _bd_ndi.binary_dilation(
                    (skin_l & core_l & halo_l).astype(bool),
                    structure=np.ones((1, 1 + 2 * skin_y, 1 + 2 * skin_x), dtype=bool),
                    iterations=1,
                )
                exclusion_l |= (skin_halo_l & core_l)
            except Exception:
                exclusion_l |= (skin_l & core_l & halo_l)

        # Keep only connected soft-tissue regions attached to tumour/skin anchors.
        try:
            anchors_l = tumor_l.astype(bool)
            if skin is not None and np.any(skin):
                anchors_l |= (skin.astype(bool)[sl] & halo_l)
            support_l = exclusion_l.astype(bool)
            grown_l = support_l & anchors_l
            if not np.any(grown_l):
                grown_l = anchors_l & support_l
            for _ in range(3):
                neigh_l = _bd_ndi.binary_dilation(grown_l.astype(bool), structure=np.ones((3, 3, 3), dtype=bool), iterations=1)
                new_l = (support_l & neigh_l) | (anchors_l & support_l)
                if np.array_equal(new_l, grown_l):
                    break
                grown_l = new_l
            exclusion_l = grown_l & support_l
        except Exception:
            pass

        try:
            exclusion_l = _bd_ndi.binary_closing(exclusion_l.astype(bool), structure=np.ones((1, 3, 3), dtype=bool), iterations=1)
            exclusion_l = _bd_remove_small_objects(exclusion_l.astype(bool), min_size=8)
        except Exception:
            pass

        tmp = np.zeros_like(ct_volume, dtype=bool)
        tmp[sl] = exclusion_l.astype(bool)
        tmp &= core
        tmp &= (((ct_volume >= float(globals().get("_BD_PERITUMORAL_SOFT_MIN_HU", -110.0))) &
                 (ct_volume <= float(globals().get("_BD_PERITUMORAL_SOFT_MAX_HU", 180.0)))) |
                tumor_in_side.astype(bool))
        return tmp.astype(bool)

    def segment_fibroglandular(self, ct_volume: np.ndarray, breast_mask: np.ndarray) -> np.ndarray:
        """Strict Boolean extraction for fibroglandular tissue.

        v36: after removing skin, pectoralis and the posterior chest-wall band,
        fibroglandular tissue is defined by a strict HU intersection. This avoids
        the previous failure mode where the whole breast ROI was painted orange/gold.
        """
        if breast_mask is None or not np.any(breast_mask):
            return np.zeros_like(ct_volume, dtype=bool)
        ...
        skin = self._skin_shell_mask(ct_volume, core)
        pectoralis = self._pectoralis_mask(ct_volume, core)
        posterior = self._posterior_band(core, start_frac=0.82, extra_voxels=2)

        # 1. Isolate true parenchymal space: inside breast ROI, but not skin,
        #    pectoralis, or posterior chest-wall band.
        parenchyma = core.astype(bool) & ~skin.astype(bool) & ~pectoralis.astype(bool) & ~posterior.astype(bool)

        # 2. Strict Boolean intersection for fibroglandular tissue.
        fg = (parenchyma &
              (ct_volume >= float(globals().get("_BD_FG_STRICT_MIN_HU", -30.0))) &
              (ct_volume <= float(globals().get("_BD_FG_STRICT_MAX_HU", 150.0))))

        # 3. Clean isolated noise voxels without dilating into fatty breast.
        try:
            fg = _bd_ndi.binary_opening(
                fg.astype(bool),
                structure=np.ones((1, 2, 2), dtype=bool),
                iterations=1
            )
            fg = _bd_remove_small_objects(fg.astype(bool), min_size=6)
        except Exception:
            pass
    ...
    def exclude_non_parenchymal(self, fg_mask: np.ndarray, ct_volume: np.ndarray,
                                breast_mask: np.ndarray | None = None) -> np.ndarray:
        """Subtract skin, pectoralis, clips/calcifications and vessels from FG."""
        if fg_mask is None or not np.any(fg_mask):
            return np.zeros_like(ct_volume, dtype=bool)
        if breast_mask is None or not np.any(breast_mask):
            try:
                breast_mask = _bd_ndi.binary_dilation(
                    fg_mask.astype(bool), structure=np.ones((3, 5, 5), dtype=bool),
                    iterations=3
                )
            except Exception:
                breast_mask = fg_mask.astype(bool)
            breast_mask &= ((ct_volume >= _BD_HU_BREAST_MIN) &
                            (ct_volume <= _BD_HU_BREAST_MAX))
        else:
            breast_mask = breast_mask.astype(bool)

        details = self.compute_exclusion_masks(ct_volume, breast_mask)
        cleaned = fg_mask.astype(bool) & breast_mask
        for key in ("skin", "pectoralis", "clips", "vessels"):
            m = details.get(key)
            if m is not None and np.any(m):
                cleaned &= ~m.astype(bool)

        cleaned &= ~self._posterior_band(breast_mask, start_frac=0.93, extra_voxels=1)
        cleaned &= _bd_lateral_side_mask(
            ct_volume.shape, _bd_side_from_mask(breast_mask),
            gap_frac=self._CENTRAL_GAP_FRAC
        )
        cleaned = _bd_morph_close_open(cleaned, close_iter=0, open_iter=0)
        try:
            cleaned = _bd_remove_small_objects(cleaned.astype(bool), min_size=5)
        except Exception:
            pass
        return cleaned.astype(bool)

    def compute_exclusion_masks(self, ct_volume: np.ndarray, breast_mask: np.ndarray) -> dict:
        """Return explicit sub-tissue masks for a single lateral breast ROI."""
        if breast_mask is None or not np.any(breast_mask):
            z = np.zeros_like(ct_volume, dtype=bool)
            return {"skin": z, "pectoralis": z, "clips": z, "vessels": z, "peritumoral": z}

        breast_mask = self._side_core(breast_mask.astype(bool), erode_iter=0)
        skin = self._skin_shell_mask(ct_volume, breast_mask)
        pectoralis = self._pectoralis_mask(ct_volume, breast_mask)
        clips = self._clips_calcification_mask(ct_volume, breast_mask)
        vessels = self._vessel_mask(ct_volume, breast_mask,
                                    skin=skin, pectoralis=pectoralis, clips=clips)
        z = np.zeros_like(ct_volume, dtype=bool)
        return {
...
class BreastCTDensityEngine:
    ...
    def compute_volumetric_density(self, ct_volume: np.ndarray,
        ...
        def _side_metrics(side: str) -> dict:
            ...
            cleaned = self.segmentor.exclude_non_parenchymal(fg_mask, ct_volume, breast_mask=breast_mask)
...
class V21BreastDensityTab(QtWidgets.QWidget):
    ...
    def __init__(self, app):
        ...
        self._fg_masks: dict = {}             # analytical fibroglandular masks per side after segmentation
        self._fg_display_masks: dict = {}     # display-thickened FG masks for live/export overlays only
        ...
        self._auto_tumor_exclusion: Optional[np.ndarray] = None  # PET-guided exophytic tumour exclusion ROI
        self._auto_peritumoral_exclusion: Optional[np.ndarray] = None  # optional automatic halo for skin thickening / stranding
        self._manual_peritumoral_include: Optional[np.ndarray] = None  # user-added editable peritumoral exclusion ROI
        self._manual_peritumoral_erase: Optional[np.ndarray] = None    # user-erased voxels from automatic/manual peritumoral ROI
        self._manual_pectoralis_right: Optional[np.ndarray] = None     # user-drawn right pectoralis ROI
        self._manual_pectoralis_left: Optional[np.ndarray] = None      # user-drawn left pectoralis ROI
        self._excl_detail_masks: dict = {}    # per-side skin/pectoralis/clips/vessels/peritumoral/manual-pectoralis (for figure export)
    ...
    def _build(self):
        ...
        peri_grp = QtWidgets.QGroupBox("Optional peritumoral exclusion halo")
        peri_lay = QtWidgets.QVBoxLayout(peri_grp)
        self._chk_enable_peritumoral = QtWidgets.QCheckBox("Enable automatic peritumoral exclusion halo")
        self._chk_enable_peritumoral.setChecked(False)
        self._chk_enable_peritumoral.setToolTip(
            "If checked, the software will build an automatic peritumoral halo around the gross tumor exclusion ROI\n"
            "to suppress cutaneous thickening and fine adjacent parenchymal stranding from the FG mask.\n"
            "Uncheck this if you want to ignore the automatic halo and rely only on manual editing."
        )
        self._chk_enable_peritumoral.toggled.connect(self._on_toggle_peritumoral_enabled)
        peri_lay.addWidget(self._chk_enable_peritumoral)

        peri_row = QtWidgets.QHBoxLayout()
        self._btn_set_peritumoral = QtWidgets.QPushButton("🟣  Set Manual Peritumoral ROI")
        self._btn_set_peritumoral.setToolTip(
            "Use the currently painted ROI as the manual peritumoral exclusion ROI (replace previous manual add ROI).\n"
            "Helpful when you want a fully manual violet mask."
        )
        self._btn_set_peritumoral.clicked.connect(lambda: self._store_manual_peritumoral_roi("replace"))
        peri_row.addWidget(self._btn_set_peritumoral)

        self._btn_add_peritumoral = QtWidgets.QPushButton("➕  Add Painted ROI to Halo")
        self._btn_add_peritumoral.setToolTip(
            "Adds the currently painted ROI to the effective peritumoral exclusion halo.\n"
            "Use this if the automatic violet halo is too small."
        )
        self._btn_add_peritumoral.clicked.connect(lambda: self._store_manual_peritumoral_roi("add"))
        peri_row.addWidget(self._btn_add_peritumoral)

        self._btn_erase_peritumoral = QtWidgets.QPushButton("➖  Erase Painted ROI from Halo")
        self._btn_erase_peritumoral.setToolTip(
            "Subtracts the currently painted ROI from the effective peritumoral exclusion halo.\n"
            "Use this if the automatic violet halo is too large or wrongly placed."
        )
        self._btn_erase_peritumoral.clicked.connect(lambda: self._store_manual_peritumoral_roi("erase"))
        peri_row.addWidget(self._btn_erase_peritumoral)

        self._btn_clear_peritumoral = QtWidgets.QPushButton("✖  Clear Peritumoral Edits")
        self._btn_clear_peritumoral.setToolTip("Clears manual add/erase edits to the peritumoral halo.")
        self._btn_clear_peritumoral.clicked.connect(self._on_clear_peritumoral_edits)
        peri_row.addWidget(self._btn_clear_peritumoral)
        peri_row.addStretch()
        peri_lay.addLayout(peri_row)
        ...
        peri_lay.addWidget(self._lbl_tumor_excl_status)

        self._lbl_peritumoral_status = QtWidgets.QLabel("Peritumoral halo: disabled by default. Enable only when needed, or use manual add/erase.")
        self._lbl_peritumoral_status.setStyleSheet("color: #7d3c98; font-style: italic;")
        peri_lay.addWidget(self._lbl_peritumoral_status)

        label_lay.addWidget(peri_grp)

        pec_grp = QtWidgets.QGroupBox("Optional manual pectoralis ROI")
        pec_lay = QtWidgets.QVBoxLayout(pec_grp)
        pec_hint = QtWidgets.QLabel(
            "Use this only when automatic pectoralis segmentation is incomplete. "
            "Paint pectoralis major/minor posterior to the breast, then store it as right or left pectoralis. "
            "Manual pectoralis is shown pink and is subtracted from fibroglandular tissue."
        )
        pec_hint.setWordWrap(True)
        pec_hint.setStyleSheet("color: #7d3c98; background: #fbf0ff; padding: 4px; border-radius: 4px;")
        pec_lay.addWidget(pec_hint)

        pec_row = QtWidgets.QHBoxLayout()
        self._btn_mark_pec_right = QtWidgets.QPushButton("💗  Mark RIGHT Pectoralis")
        self._btn_mark_pec_right.setToolTip(
            "Paint the right pectoralis muscle in the main viewer, then click this. "
            "It will be shown in pink and subtracted from right FG."
        )
        self._btn_mark_pec_right.clicked.connect(lambda: self._on_label_pectoralis("right"))
        pec_row.addWidget(self._btn_mark_pec_right)

        self._btn_mark_pec_left = QtWidgets.QPushButton("💗  Mark LEFT Pectoralis")
        self._btn_mark_pec_left.setToolTip(
            "Paint the left pectoralis muscle in the main viewer, then click this. "
            "It will be shown in pink and subtracted from left FG."
        )
        self._btn_mark_pec_left.clicked.connect(lambda: self._on_label_pectoralis("left"))
        pec_row.addWidget(self._btn_mark_pec_left)

        self._btn_clear_pec = QtWidgets.QPushButton("✖  Clear Pectoralis ROI")
        self._btn_clear_pec.setToolTip("Clear stored manual pectoralis ROIs.")
        self._btn_clear_pec.clicked.connect(self._on_clear_pectoralis)
        pec_row.addWidget(self._btn_clear_pec)
        pec_row.addStretch()
        pec_lay.addLayout(pec_row)

        self._lbl_pec_status = QtWidgets.QLabel("No manual pectoralis ROI set.")
        self._lbl_pec_status.setStyleSheet("color: #7d3c98; font-style: italic;")
        pec_lay.addWidget(self._lbl_pec_status)

        label_lay.addWidget(pec_grp)
        ...
        self._btn_segment_fg = QtWidgets.QPushButton("🔬  Segment FG + Manual ROI Density")
        self._btn_segment_fg.setEnabled(BREAST_DENSITY_DEPS_AVAILABLE)
        self._btn_segment_fg.setToolTip(
            "Stable no-freeze route: computes whole-breast volume from your labelled ROIs\n"
            "and fibroglandular volume from cleaned soft-tissue mask.\n\n"
        ...
        self._btn_density = QtWidgets.QPushButton("▶  Compute Density Auto (optional)")
        self._btn_density.setEnabled(BREAST_DENSITY_DEPS_AVAILABLE)
        self._btn_density.setToolTip(
            "Optional automatic whole-breast segmentation. Use only when no manual breast ROIs are available.\n"
            "If manual ROIs exist, this button redirects to the stable manual ROI density route."
        )
        ...
        self._btn_export_fig.setToolTip(
            ...
            "  Peritumoural exclusion halo — violet\n\n"
            "Run 'Segment Fibroglandular Tissue' or 'Compute Density (Auto)' first."
        )
        self._btn_export_fig.clicked.connect(self._on_export_figure)
        btn_row.addWidget(self._btn_export_fig)

        self._btn_export_training = QtWidgets.QPushButton("🧠  Save Training Case")
        self._btn_export_training.setToolTip(
            "Save CT/PET volumes, final labelled masks, metadata.json, and QC PNG into a training case folder."
        )
        self._btn_export_training.clicked.connect(self._on_export_training_case)
        btn_row.addWidget(self._btn_export_training)
        btn_row.addStretch()
    ...
    def _set_busy(self, busy: bool, message: str = "") -> None:
        ...
        action_btns = [
            ...
            getattr(self, "_btn_export_training", None),
            getattr(self, "_btn_mark_pec_right", None), getattr(self, "_btn_mark_pec_left", None),
            getattr(self, "_btn_clear_pec", None), getattr(self, "_btn_set_peritumoral", None),
            getattr(self, "_btn_add_peritumoral", None), getattr(self, "_btn_erase_peritumoral", None),
            getattr(self, "_btn_clear_peritumoral", None),
        ]
        if busy:
            ...
            for btn in (
                ...
                getattr(self, "_btn_clear_pec", None), getattr(self, "_btn_set_peritumoral", None),
                getattr(self, "_btn_add_peritumoral", None), getattr(self, "_btn_erase_peritumoral", None),
                getattr(self, "_btn_clear_peritumoral", None), getattr(self, "_btn_export_training", None),
            ):
                if btn is not None:
                    btn.setEnabled(True)

    # ------------------------------------------------------------------
    def _base_tumor_exclusion_mask(self):
        """Return gross tumour exclusion, protected against accidental pectoralis overlap."""
        out = None
        for m in (self._roi_tumor_exclusion, self._auto_tumor_exclusion):
            if m is not None and np.any(m):
                out = m.astype(bool).copy() if out is None else (out | m.astype(bool))
        pec = self._manual_pectoralis_mask()
        if out is not None and pec is not None and np.any(pec) and pec.shape == out.shape:
            out = out & ~pec.astype(bool)
        return out if out is not None and np.any(out) else None

    def _effective_peritumoral_exclusion_mask(self):
        """Return the currently active peritumoral halo after enable/disable and manual edits."""
        out = None
        use_auto = bool(getattr(self, "_chk_enable_peritumoral", None).isChecked()) if hasattr(self, "_chk_enable_peritumoral") else True
        if use_auto and self._auto_peritumoral_exclusion is not None and np.any(self._auto_peritumoral_exclusion):
            out = self._auto_peritumoral_exclusion.astype(bool).copy()
        if self._manual_peritumoral_include is not None and np.any(self._manual_peritumoral_include):
            out = self._manual_peritumoral_include.astype(bool).copy() if out is None else (out | self._manual_peritumoral_include.astype(bool))
        if self._manual_peritumoral_erase is not None and np.any(self._manual_peritumoral_erase):
            if out is None:
                out = np.zeros_like(self._manual_peritumoral_erase, dtype=bool)
            out &= ~self._manual_peritumoral_erase.astype(bool)
        return out if out is not None and np.any(out) else None

    def _effective_peritumoral_exclusion_for_side(self, side: str | None = None):
        """Return active peritumoral halo, optionally limited to one lateral side."""
        out = self._effective_peritumoral_exclusion_mask()
        if out is None or not np.any(out):
            return None
        if side in ("right", "left"):
            try:
                out = out & _bd_lateral_side_mask(out.shape, side, gap_frac=max(getattr(self, '_CENTRAL_GAP_FRAC', 0.10), 0.10))
            except Exception:
                pass
        return out if np.any(out) else None

    def _combined_tumor_exclusion_mask(self):
        """Gross tumour exclusion plus currently active peritumoural halo."""
        out = self._base_tumor_exclusion_mask()
        peri = self._effective_peritumoral_exclusion_mask()
        if peri is not None and np.any(peri):
            out = peri.astype(bool).copy() if out is None else (out | peri.astype(bool))
        return out if out is not None and np.any(out) else None

    def _auto_pet_tumor_exclusion(self, candidate_roi: np.ndarray | None = None) -> np.ndarray | None:
        """PET-guided mask for large exophytic FDG-avid breast tumour.

        v26: Uses a broad anterior-thoracic search region when the labelled
        breast ROI is too small, then grows the hottest connected component.
        This specifically addresses exophytic tumours protruding outside the
        breast mound and therefore missed by CT-density thresholds alone.
        """

        if not globals().get("_BD_ENABLE_PET_AUTO_TUMOR_EXCLUSION", False):
            return None

        pet = getattr(self._app, "pet_suv", None)
        ct = getattr(self._app, "ct_hu", None)
        if pet is None or ct is None or not hasattr(pet, "shape") or pet.shape != ct.shape:
            return None

        finite_pet = np.isfinite(pet)
        if not np.any(finite_pet):
            return None
        ...
        roi = None
        if candidate_roi is not None and np.any(candidate_roi):
            try:
                roi = _bd_ndi.binary_dilation(
                    candidate_roi.astype(bool),
                    structure=np.ones((7, 31, 31), dtype=bool),
                    iterations=2,
                )
            except Exception:
                roi = candidate_roi.astype(bool).copy()

        try:
            z_top, z_bot = _bd_detect_thorax_z_range(ct)
        except Exception:
            z_top, z_bot = 0, int(nz * 0.65)
        y_end = int(ny * 0.66)
        broad = np.zeros_like(ct, dtype=bool)
        broad[max(0, z_top):min(nz, z_bot + 1), :max(1, y_end), :] = True
        broad &= body
        broad &= _bd_lateral_side_mask(ct.shape, None, gap_frac=0.06)
        broad &= (ct > -450) & (ct < 260)

        if roi is None or not np.any(roi):
            roi = broad
        else:
            supplement = np.zeros_like(broad, dtype=bool)
            if np.any(broad & finite_pet):
                try:
                    supplement = broad & (pet >= np.nanpercentile(pet[broad & finite_pet], 99.5))
                except Exception:
                    supplement = np.zeros_like(broad, dtype=bool)
            roi = (roi & body & (ct > -500) & (ct < 300)) | supplement
        ...
        high_thr = max(3.0, roi_max * 0.55)
        core = (pet >= high_thr) & roi
        if not np.any(core):
            high_thr = max(2.5, float(np.nanpercentile(vals, 99.3)))
            core = (pet >= high_thr) & roi
        if not np.any(core):
            return None

        labels, n = _bd_ndi.label(core.astype(bool))
        if n == 0:
            return None
        sizes = np.bincount(labels.ravel())
        sizes[0] = 0
        ...
        for lbl in range(1, n + 1):
            ...
            if comp_vals.size == 0:
                continue
            zz, yy, xx = np.nonzero(comp)
            score = float(np.nanmax(comp_vals)) * (1.0 + np.log1p(size))
            if float(yy.mean()) > ny * 0.70:
                score *= 0.3
            if abs(float(xx.mean()) - nx / 2.0) < nx * 0.06:
                score *= 0.2
            if score > best_score:
                best_score = score
                best_lbl = lbl
        ...
        seed = labels == best_lbl
        comp_max = float(np.nanmax(pet[seed]))
        grow_thr = max(2.0, comp_max * 0.30)
        grow_cand = (pet >= grow_thr) & roi
        labels2, n2 = _bd_ndi.label(grow_cand.astype(bool))
        out = seed.copy()
        if n2 > 0:
            overlap_ids = np.unique(labels2[seed])
            overlap_ids = overlap_ids[overlap_ids > 0]
            if overlap_ids.size:
                out = np.isin(labels2, overlap_ids)

        try:
            out = _bd_ndi.binary_dilation(out, structure=np.ones((3, 5, 5), dtype=bool), iterations=1)
            out &= roi
            out = _bd_ndi.binary_closing(out, structure=np.ones((3, 3, 3), dtype=bool), iterations=1)
            out = _bd_ndi.binary_fill_holes(out)
        except Exception:
            pass

        if int(np.sum(out)) < 20:
            return None
        return out.astype(bool)

    def _refresh_auto_tumor_exclusion(self, candidate_roi: np.ndarray | None = None) -> None:
        # v27: disabled by default because full-volume PET thresholding/dilation
        # was freezing the UI before the worker thread started.  The manually
        # painted tumour exclusion ROI remains fully supported and is preferred.
        if not globals().get("_BD_ENABLE_PET_AUTO_TUMOR_EXCLUSION", False):
            self._auto_tumor_exclusion = None
            return
        auto = self._auto_pet_tumor_exclusion(candidate_roi)
        self._auto_tumor_exclusion = auto if auto is not None and np.any(auto) else None
        if self._auto_tumor_exclusion is not None:
            self._log_msg(f"[Tumour] PET-guided exophytic tumour mask detected — "
                          f"{int(np.sum(self._auto_tumor_exclusion))} voxels will be excluded from FG.")

    def _refresh_peritumoral_exclusion(self, whole_masks: dict | None = None, ct_volume: np.ndarray | None = None, ct_sitk=None) -> None:
        """Build the optional peritumoural exclusion halo from the stored tumour ROI."""
        if not globals().get("_BD_ENABLE_AUTO_PERITUMORAL_EXCLUSION", True):
            self._auto_peritumoral_exclusion = None
            return
        base = self._base_tumor_exclusion_mask()
        if base is None or not np.any(base):
            self._auto_peritumoral_exclusion = None
            return
        if ct_volume is None:
            ct_volume = getattr(self._app, "ct_hu", None)
        if ct_volume is None or not hasattr(ct_volume, "shape"):
            self._auto_peritumoral_exclusion = None
            return
        if ct_sitk is None:
            ct_sitk = getattr(self._app, "ct_image", None)
        # Keep this helper fast and GUI-safe.  It must not call TotalSegmentator.
        segmentor = BreastSegmentor(None, use_total_segmentator=False)
        masks = whole_masks or {"right_mask": self._roi_right, "left_mask": self._roi_left}
        out = None
        for side in ("right", "left"):
            breast_mask = masks.get(f"{side}_mask") if isinstance(masks, dict) else None
            if breast_mask is None or not np.any(breast_mask):
                breast_mask = self._roi_right if side == "right" else self._roi_left
            if breast_mask is None or not np.any(breast_mask):
                continue
            details = segmentor.compute_exclusion_masks(ct_volume, breast_mask)
            peri = segmentor.build_peritumoral_exclusion(ct_volume, breast_mask, base, details=details)
            if peri is not None and np.any(peri):
                out = peri.astype(bool).copy() if out is None else (out | peri.astype(bool))
        self._auto_peritumoral_exclusion = out if out is not None and np.any(out) else None
        if self._auto_peritumoral_exclusion is not None:
            self._log_msg(f"[Tumour] Optional peritumoural exclusion halo updated — {int(np.sum(self._auto_peritumoral_exclusion))} voxels.")

    def _manual_pectoralis_mask(self, side: str | None = None):
        """Return the stored manual pectoralis mask, optionally for one side."""
        if side == "right":
            return self._manual_pectoralis_right
        if side == "left":
            return self._manual_pectoralis_left
        out = None
        for m in (self._manual_pectoralis_right, self._manual_pectoralis_left):
            if m is not None and np.any(m):
                out = m.astype(bool).copy() if out is None else (out | m.astype(bool))
        return out if out is not None and np.any(out) else None

    def _effective_pectoralis_for_side(self, side: str, auto_mask=None):
        """Manual pectoralis overrides automatic pectoralis for this side."""
        manual = self._manual_pectoralis_mask(side)
        if manual is not None and np.any(manual):
            return manual.astype(bool)
        if auto_mask is not None and np.any(auto_mask):
            return auto_mask.astype(bool)
        return None

    def _on_label_pectoralis(self, side: str):
        """Snapshot current viewer paint mask as manual pectoralis for one side."""
        snap = self._get_drawn_viewer_mask()
        if snap is None:
            QtWidgets.QMessageBox.warning(
                self, "Manual Pectoralis ROI",
                "No ROI drawn in the main viewer yet.\n\n"
                "Switch to Paint or Lasso mode, draw the pectoralis muscle posterior to the breast, then click this button."
            )
            return

        # Keep the stored pectoralis on the requested lateral side when possible.
        try:
            side_mask = _bd_lateral_side_mask(snap.shape, side, gap_frac=0.06)
            snap = snap.astype(bool) & side_mask
        except Exception:
            snap = snap.astype(bool)

        # Safety fix: pectoralis must never be stored as tumour or peritumoural tissue.
        # If the active drawing canvas accidentally still contains older tumour/peritumoural
        # paint, remove those components before saving the pink pectoralis mask.
        for _m in (self._base_tumor_exclusion_mask(), self._effective_peritumoral_exclusion_mask()):
            if _m is not None and np.any(_m) and _m.shape == snap.shape:
                snap &= ~_m.astype(bool)

        if not np.any(snap):
            QtWidgets.QMessageBox.warning(
                self, "Manual Pectoralis ROI",
                "The stored pectoralis ROI became empty after removing tumour/peritumoural overlap.\n\n"
                "Clear viewer paint, repaint pectoralis only, then click the pectoralis button again."
            )
            return

        if side == "right":
            self._manual_pectoralis_right = snap
        else:
            self._manual_pectoralis_left = snap

        self._log_msg(f"[Pectoralis] Manual {side.upper()} pectoralis ROI stored — {int(np.sum(snap))} voxels.")
        self._update_roi_status()
        self._push_seg_overlays()
        self._clear_viewer_paint_silent()
        self._log_msg("[Paint] Viewer paint auto-cleared after storing pectoralis ROI.")

    def _on_clear_pectoralis(self):
        """Clear stored manual pectoralis ROIs."""
        self._manual_pectoralis_right = None
        self._manual_pectoralis_left = None
        self._log_msg("[Pectoralis] Manual pectoralis ROIs cleared.")
        self._update_roi_status()
        self._push_seg_overlays()

    def _get_drawn_viewer_mask(self):
        """Return the currently painted ROI in the main viewer as a boolean snapshot."""
        ...
        if roi_mask is None or not np.any(roi_mask):
            return None
        return roi_mask.astype(bool).copy()

    def _on_toggle_peritumoral_enabled(self, checked: bool):
        state = "enabled" if checked else "disabled"
        self._log_msg(f"[Peritumoral] Automatic halo {state}.")
        self._update_roi_status()
        self._push_seg_overlays()

    def _store_manual_peritumoral_roi(self, mode: str = "add"):
        """Replace/add/erase the manual peritumoral halo using the current viewer paint mask."""
        snap = self._get_drawn_viewer_mask()
        if snap is None:
            QtWidgets.QMessageBox.warning(
                self, "Peritumoral ROI",
                "No ROI drawn in the main viewer yet.\n\n"
                "Switch to Paint or Lasso mode, draw the desired peritumoral region, then try again."
            )
            return
        # Safety fix: do not allow manual pectoralis masks to become part of the violet halo.
        pec_union = self._manual_pectoralis_mask()
        if pec_union is not None and np.any(pec_union) and pec_union.shape == snap.shape:
            snap = snap.astype(bool) & ~pec_union.astype(bool)
        else:
            snap = snap.astype(bool)

        if mode == "replace":
            self._manual_peritumoral_include = snap.astype(bool)
            self._manual_peritumoral_erase = None
            self._log_msg(f"[Peritumoral] Manual peritumoral ROI replaced — {int(np.sum(snap))} voxels.")
        elif mode == "add":
            cur = self._manual_peritumoral_include
            self._manual_peritumoral_include = snap.astype(bool).copy() if cur is None else (cur.astype(bool) | snap.astype(bool))
            if self._manual_peritumoral_erase is not None and np.any(self._manual_peritumoral_erase):
                self._manual_peritumoral_erase &= ~snap.astype(bool)
            self._log_msg(f"[Peritumoral] Painted ROI added to peritumoral halo — +{int(np.sum(snap))} voxels requested.")
        elif mode == "erase":
            cur = self._manual_peritumoral_erase
            self._manual_peritumoral_erase = snap.astype(bool).copy() if cur is None else (cur.astype(bool) | snap.astype(bool))
            if self._manual_peritumoral_include is not None and np.any(self._manual_peritumoral_include):
                self._manual_peritumoral_include &= ~snap.astype(bool)
            self._log_msg(f"[Peritumoral] Painted ROI erased from peritumoral halo — {int(np.sum(snap))} voxels marked for removal.")
        else:
            return
        self._update_roi_status()
        self._push_seg_overlays()
        self._clear_viewer_paint_silent()
        self._log_msg("[Paint] Viewer paint auto-cleared after storing peritumoral edit.")

    def _on_clear_peritumoral_edits(self):
        """Clear manual add/erase edits for the peritumoral halo."""
        self._manual_peritumoral_include = None
        self._manual_peritumoral_erase = None
        self._log_msg("[Peritumoral] Manual add/erase edits cleared.")
        self._update_roi_status()
        self._push_seg_overlays()
    ...
    def _update_roi_status(self):
        ...
        base_tumor = self._base_tumor_exclusion_mask()
        effective_peri = self._effective_peritumoral_exclusion_mask()
        combined = self._combined_tumor_exclusion_mask()
        manual_n = int(np.sum(self._roi_tumor_exclusion)) if self._roi_tumor_exclusion is not None else 0
        auto_n = int(np.sum(self._auto_tumor_exclusion)) if self._auto_tumor_exclusion is not None else 0
        if base_tumor is not None:
            self._lbl_tumor_excl_status.setText(
                f"🚫 Gross tumor exclusion set (manual={manual_n} vox, PET-auto={auto_n} vox)"
            )
        else:
            self._lbl_tumor_excl_status.setText("No gross tumor exclusion ROI set.")

        peri_auto_n = int(np.sum(self._auto_peritumoral_exclusion)) if self._auto_peritumoral_exclusion is not None else 0
        peri_add_n = int(np.sum(self._manual_peritumoral_include)) if self._manual_peritumoral_include is not None else 0
        peri_erase_n = int(np.sum(self._manual_peritumoral_erase)) if self._manual_peritumoral_erase is not None else 0
        peri_eff_n = int(np.sum(effective_peri)) if effective_peri is not None else 0
        enabled = bool(getattr(self, "_chk_enable_peritumoral", None).isChecked()) if hasattr(self, "_chk_enable_peritumoral") else True
        if combined is None:
            self._lbl_peritumoral_status.setText(
                f"Peritumoral halo: {'enabled' if enabled else 'disabled'}; no active exclusion mask yet."
            )
        else:
            self._lbl_peritumoral_status.setText(
                f"🟣 Peritumoral halo {'enabled' if enabled else 'disabled'} "
                f"(auto={peri_auto_n} vox, manual-add={peri_add_n} vox, manual-erase={peri_erase_n} vox, effective={peri_eff_n} vox)"
            )

        if hasattr(self, "_lbl_pec_status"):
            r_pec = int(np.sum(self._manual_pectoralis_right)) if self._manual_pectoralis_right is not None else 0
            l_pec = int(np.sum(self._manual_pectoralis_left)) if self._manual_pectoralis_left is not None else 0
            if r_pec or l_pec:
                self._lbl_pec_status.setText(f"💗 Manual pectoralis ROI: right={r_pec} vox, left={l_pec} vox")
            else:
                self._lbl_pec_status.setText("No manual pectoralis ROI set. Automatic pectoralis/heuristic will be used if available.")


    def _build_fg_display_mask(self, fg_mask: np.ndarray | None, roi_mask: np.ndarray | None, detail_masks: dict | None = None, tumor_mask: np.ndarray | None = None) -> np.ndarray | None:
        """Return a display-only FG mask thickened for visibility, without changing density calculations."""
        if fg_mask is None or not np.any(fg_mask):
            return None
        disp = fg_mask.astype(bool).copy()
        try:
            disp = _bd_ndi.binary_dilation(
                disp,
                structure=np.ones((1, 3, 3), dtype=bool),
                iterations=max(1, int(globals().get("_BD_FG_DISPLAY_THICKEN_ITER", 1)))
            )
        except Exception:
            pass
        if roi_mask is not None and np.any(roi_mask):
            disp &= roi_mask.astype(bool)
        if isinstance(detail_masks, dict):
            for _k in ("skin", "pectoralis", "clips", "vessels", "peritumoral"):
                _m = detail_masks.get(_k)
                if _m is not None and np.any(_m):
                    disp &= ~_m.astype(bool)
        if tumor_mask is not None and np.any(tumor_mask):
            disp &= ~tumor_mask.astype(bool)
        return disp.astype(bool) if np.any(disp) else fg_mask.astype(bool)

    def _refresh_fg_display_masks(self):
        """Refresh display-thickened FG masks from analytical FG masks."""
        display = {}
        tumor_mask = self._base_tumor_exclusion_mask()
        for side in ("right", "left"):
            fg = self._fg_masks.get(side) if isinstance(self._fg_masks, dict) else None
            if fg is None or not np.any(fg):
                continue
            roi = None
            if isinstance(self._whole_masks, dict):
                roi = self._whole_masks.get(f"{side}_mask")
            if roi is None:
                roi = self._roi_right if side == "right" else self._roi_left
            details = self._excl_detail_masks.get(side) if isinstance(self._excl_detail_masks, dict) else None
            disp = self._build_fg_display_mask(fg, roi, details, tumor_mask)
            if disp is not None and np.any(disp):
                display[side] = disp.astype(bool)
        self._fg_display_masks = display

    def _push_seg_overlays(self):
        """Push colour-coded segmentation masks using strict z-order hierarchy."""
        overlays: dict = {}
        ...
        def _union_detail(key: str):
            acc = None
            for side in ("right", "left"):
                d = self._excl_detail_masks.get(side) if self._excl_detail_masks else None
                m = d.get(key) if d else None
                if m is None:
                    continue
                acc = m.astype(bool) if acc is None else (acc | m.astype(bool))
            return acc

        skin = _union_detail("skin")
        pec = _union_detail("pectoralis")
        manual_pec = self._manual_pectoralis_mask()
        if manual_pec is not None:
            pec = manual_pec.astype(bool) if pec is None else (pec.astype(bool) | manual_pec.astype(bool))
        clips = _union_detail("clips")
        vessels = _union_detail("vessels")
        peritumoral = self._effective_peritumoral_exclusion_mask()

        # Layer 1: background whole breast.
        if right_whole is not None:
            overlays[(0, 200, 200, 45)] = right_whole
        if left_whole is not None:
            overlays[(70, 130, 180, 45)] = left_whole

        # Layer 2: pectoralis muscle, above whole-breast mask.
        if pec is not None:
            overlays[(255, 105, 180, 175)] = pec

        # Layer 3: skin shell.
        if skin is not None:
            overlays[(107, 142, 35, 175)] = skin

        # Layer 4: fibroglandular tissue, above background but below exclusions.
        # Use display-thickened masks for visibility only; density metrics still use self._fg_masks.
        fg_for_display = self._fg_display_masks if isinstance(self._fg_display_masks, dict) and self._fg_display_masks else self._fg_masks
        if "right" in fg_for_display:
            overlays[(255, 140, 0, 220)] = fg_for_display["right"]
        if "left" in fg_for_display:
            overlays[(220, 200, 0, 220)] = fg_for_display["left"]

        # Layer 5: high-density exclusions.
        if vessels is not None:
            overlays[(0, 230, 115, 200)] = vessels
        if clips is not None:
            overlays[(255, 255, 0, 225)] = clips

        # Layer 6: optional peritumoural exclusion halo.
        if peritumoral is not None:
            overlays[(160, 32, 240, 185)] = peritumoral

        # Layer 7: gross tumour exclusion, absolute top priority.
        tumor_union = self._base_tumor_exclusion_mask()
        if tumor_union is not None:
            overlays[(255, 0, 0, 255)] = tumor_union
    ...
    def _make_kv_table(self) -> QtWidgets.QTableWidget:
    ...
    def _on_label_roi(self, side: str):
        ...
        self._update_roi_status()
        # Critical safety fix: clear the active drawing canvas after storing this ROI.
        # This prevents a later pectoralis/peritumoral/tumour paint from being accidentally
        # unioned into the previously stored label.
        self._clear_viewer_paint_silent()
        self._log_msg(f"[Paint] Viewer paint auto-cleared after storing {side.upper()} breast ROI.")
    ...
    def _clear_viewer_paint_silent(self):
        """Zero the active drawing canvas without touching stored breast/tumour/peritumoral/pectoralis ROIs."""
        mask = getattr(self._app, "mask", None)
        if mask is not None:
            mask[:] = False
        mask_primary = getattr(self._app, "mask_primary", None)
    ...
    def _on_label_tumor_exclusion(self):
        ...
        # v28: accept the user's manually painted tumour exclusion exactly as drawn.
        # Large exophytic breast tumours can occupy a substantial part of the breast
        # mound, so the previous >85% rejection could prevent a valid tumour ROI
        # from being stored. The ROI is still only used as an exclusion layer; it
        # does not alter the stored whole-breast ROI.

        # Safety fix: tumour exclusion must not accidentally include already stored manual pectoralis.
        pec_union = self._manual_pectoralis_mask()
        if pec_union is not None and np.any(pec_union) and pec_union.shape == snap.shape:
            snap = snap.astype(bool) & ~pec_union.astype(bool)
        ...
        self._auto_peritumoral_exclusion = None
        self._refresh_auto_tumor_exclusion(snap)
        # Do NOT build the automatic peritumoral halo here.  Doing so can trigger
        # TotalSegmentator/model work on the GUI thread and freeze the viewer after
        # a long manual ROI drawing session.  The halo is built inside the background
        # Segment FG worker and cached afterward.
        self._log_msg(f"[ROI] Tumor exclusion ROI set — {int(np.sum(snap))} voxels will be subtracted from FG mask. Peritumoral halo will be built during Segment FG.")
        self._update_roi_status()
        self._push_seg_overlays()
        self._clear_viewer_paint_silent()
        self._log_msg("[Paint] Viewer paint auto-cleared after storing tumour exclusion ROI.")
    ...
    def _on_segment_fg(self):
        ...
        roi_union = None
        for _r in (roi_right, roi_left):
            if _r is not None:
                roi_union = _r.astype(bool).copy() if roi_union is None else (roi_union | _r.astype(bool))
        self._refresh_auto_tumor_exclusion(roi_union)
        tumor_union = self._base_tumor_exclusion_mask()
        tumor_excl = tumor_union.copy() if tumor_union is not None else None
        ct_snap = ct_hu.copy()
        ct_sitk_ref = getattr(self._app, "ct_image", None)
        # FINAL STABLE PATH: never run TotalSegmentator from the manual Segment FG button.
        # TotalSegmentator is useful for optional auto-anatomy, but it has no true
        # fibroglandular-tissue class in v2.12.0 and can freeze the workflow if called here.
        use_ts_manual = False
        use_auto_peri = bool(self._chk_enable_peritumoral.isChecked()) if hasattr(self, "_chk_enable_peritumoral") else False
        peri_add = self._manual_peritumoral_include.copy() if self._manual_peritumoral_include is not None else None
        peri_erase = self._manual_peritumoral_erase.copy() if self._manual_peritumoral_erase is not None else None
        manual_pec_right = self._manual_pectoralis_right.copy() if self._manual_pectoralis_right is not None else None
        manual_pec_left = self._manual_pectoralis_left.copy() if self._manual_pectoralis_left is not None else None

        def _compute():
            segmentor = BreastSegmentor(ct_sitk_ref if use_ts_manual else None, use_total_segmentator=use_ts_manual)
            results: dict = {}
            fg_masks: dict = {}
            excl_detail: dict = {}
            for side, roi in (("right", roi_right), ("left", roi_left)):
                ...
                # Compute detail masks once.  The old code called
                # compute_exclusion_masks() inside exclude_non_parenchymal() and
                # then called it a second time for overlays, doubling the slowest
                # connected-component work.
                fg_raw = segmentor.segment_fibroglandular(ct_snap, roi)
                details = segmentor.compute_exclusion_masks(ct_snap, roi)

                manual_pec = manual_pec_right if side == "right" else manual_pec_left
                if manual_pec is not None and np.any(manual_pec):
                    details["pectoralis"] = manual_pec.astype(bool)

                peri_eff = np.zeros_like(ct_snap, dtype=bool)
                if use_auto_peri and tumor_excl is not None and np.any(tumor_excl):
                    peri_eff |= segmentor.build_peritumoral_exclusion(ct_snap, roi, tumor_excl, details=details).astype(bool)
                if peri_add is not None and np.any(peri_add):
                    peri_eff |= (peri_add.astype(bool) & roi.astype(bool))
                if peri_erase is not None and np.any(peri_erase):
                    peri_eff &= ~(peri_erase.astype(bool) & roi.astype(bool))
                details["peritumoral"] = peri_eff.astype(bool)
                fg_clean = fg_raw.astype(bool) & roi.astype(bool)
                for _k in ("skin", "pectoralis", "clips", "vessels", "peritumoral"):
                    _m = details.get(_k)
                    if _m is not None and np.any(_m):
                        fg_clean &= ~_m.astype(bool)
                # Remove only the far posterior rim; pectoralis itself is subtracted via the explicit pink mask.
                fg_clean &= ~segmentor._posterior_band(roi, start_frac=0.97, extra_voxels=0)
                ...
                # Apply gross tumour exclusion zone
                if tumor_excl is not None:
                    fg_clean = fg_clean & ~tumor_excl
                # Make thin true fibroglandular strands visually legible while
                # keeping them inside the breast ROI and away from tumour/skin/
                # pectoralis/vessels/clips.
                try:
                    fg_clean = _bd_ndi.binary_dilation(
                        fg_clean.astype(bool),
                        structure=np.ones((1, 2, 2), dtype=bool),
                        iterations=max(0, int(_BD_FG_VISUAL_THICKEN_ITER))
                    )
                    fg_clean &= roi.astype(bool)
                    for _k in ("skin", "pectoralis", "clips", "vessels", "peritumoral"):
                        _m = details.get(_k)
                        if _m is not None and np.any(_m):
                            fg_clean &= ~_m.astype(bool)
                    if tumor_excl is not None:
                        fg_clean &= ~tumor_excl.astype(bool)
                except Exception:
                    pass
                try:
                    fg_clean = _bd_remove_small_objects(fg_clean.astype(bool), min_size=5)
                except Exception:
                    pass
                fg_vol_cc = float(np.sum(fg_clean)) * vox_cc
                fat_vol_cc = max(0.0, whole_vol_cc - fg_vol_cc)
                fg_masks[side] = fg_clean.astype(bool)
                excl_detail[side] = details
                results.update({
            ...
            whole_masks = {"right_mask": roi_right.astype(bool) if roi_right is not None else None,
                           "left_mask": roi_left.astype(bool) if roi_left is not None else None}
            return {"results": results, "fg_masks": fg_masks, "excl_detail": excl_detail,
                    "whole_masks": whole_masks}

        mode_note = "FINAL STABLE manual ROI density path — no TotalSegmentator call"
        self._log_msg(f"[FG] Running {mode_note} — {BREAST_SEGMENTATION_STRICT_VERSION} …")
    ...
    def _set_cached_peritumoral_from_details(self, detail_masks: dict | None) -> None:
        """Cache the union of per-side peritumoral masks already computed by the worker.

        This avoids re-running peritumoral construction or TotalSegmentator on the
        GUI thread after segmentation finishes.
        """
        out = None
        if isinstance(detail_masks, dict):
            for side in ("right", "left"):
                d = detail_masks.get(side)
    ...
    def _on_segment_fg_done(self, payload: dict):
        ...
        self._set_cached_peritumoral_from_details(self._excl_detail_masks)
        self._push_seg_overlays()
        for side, mask in fg_masks.items():
            ...
            if not np.any(mask):
                self._log_msg(f"[FG] WARNING: {side.upper()} analytical FG mask is empty; no FG overlay can be shown for that side.")
        self._update_roi_status()
        self._log_msg("[FG] Done. Colour overlays: cyan=right whole, blue=left whole, "
                      "orange=right FG, gold=left FG, olive=skin, magenta=pectoralis, "
                      "yellow=clips/calcifications, green=vessels, violet=peritumoural halo. "
                      "Stable manual ROI density path used: breast ROIs provide whole-breast volume; "
                      "tumour, peritumoral edits and manual pectoralis are subtracted from FG. "
                      "TotalSegmentator 2.12.0 has no dedicated fibroglandular-tissue class, so FG remains "
                      "strict CT soft-tissue extraction (-45 to 150 HU) inside the cleaned breast ROI.")
    ...
    def _on_compute_density(self):
        ...
        if ct_hu is None:
            ...
            return
        if globals().get("_BD_PREFER_MANUAL_ROI_DENSITY_WHEN_AVAILABLE", True) and \
                (self._roi_left is not None or self._roi_right is not None):
            self._log_msg("[Density] Manual labelled breast ROI(s) found — using fast manual ROI density mode; "
                          "auto whole-breast segmentation and TotalSegmentator are skipped for safety.")
            self._on_segment_fg()
            return
        ...
        self._refresh_auto_tumor_exclusion(None)
        tumor_union = self._base_tumor_exclusion_mask()
        tumor_excl = tumor_union.copy() if tumor_union is not None else None
        use_auto_peri = bool(self._chk_enable_peritumoral.isChecked()) if hasattr(self, "_chk_enable_peritumoral") else True
        peri_add = self._manual_peritumoral_include.copy() if self._manual_peritumoral_include is not None else None
        peri_erase = self._manual_peritumoral_erase.copy() if self._manual_peritumoral_erase is not None else None
        manual_pec_right = self._manual_pectoralis_right.copy() if self._manual_pectoralis_right is not None else None
        manual_pec_left = self._manual_pectoralis_left.copy() if self._manual_pectoralis_left is not None else None
        ...
        def _compute():
            segmentor = BreastSegmentor(ct_sitk_ref, use_total_segmentator=True)
            ...
            for side in ("right", "left"):
                ...
                details = segmentor.compute_exclusion_masks(ct_snap, breast_mask)
                ...
                peri_eff = np.zeros_like(ct_snap, dtype=bool)
                if use_auto_peri and tumor_excl is not None and np.any(tumor_excl):
                    peri_eff |= segmentor.build_peritumoral_exclusion(ct_snap, breast_mask, tumor_excl, details=details).astype(bool)
                if peri_add is not None and np.any(peri_add):
                    peri_eff |= (peri_add.astype(bool) & breast_mask.astype(bool))
                if peri_erase is not None and np.any(peri_erase):
                    peri_eff &= ~(peri_erase.astype(bool) & breast_mask.astype(bool))
                details["peritumoral"] = peri_eff.astype(bool)
                fg_clean = fg_raw.astype(bool) & breast_mask.astype(bool)
                for _k in ("skin", "pectoralis", "clips", "vessels", "peritumoral"):
                    _m = details.get(_k)
                    if _m is not None and np.any(_m):
                        fg_clean &= ~_m.astype(bool)
                fg_clean &= ~segmentor._posterior_band(breast_mask, start_frac=0.93, extra_voxels=1)
                # Apply gross tumour exclusion zone
                if tumor_excl is not None:
                    fg_clean = fg_clean & ~tumor_excl
                # Make thin true fibroglandular strands visually legible while
                ...
                try:
                    ...
                    fg_clean &= breast_mask.astype(bool)
                    for _k in ("skin", "pectoralis", "clips", "vessels", "peritumoral"):
                        _m = details.get(_k)
                        if _m is not None and np.any(_m):
                            fg_clean &= ~_m.astype(bool)
        ...
        if _BD_TS_AVAILABLE:
            ts_note = " (TotalSegmentator — auto mode; first run may take time for model loading)"
        else:
        ...
        self._log_msg(f"[Density] Starting auto-segmentation{ts_note} — {BREAST_SEGMENTATION_STRICT_VERSION} …")
    ...
    def _on_density_done(self, payload: dict):
        ...
        self._log_msg(f"[Density] Done. Segmentation backend: {backend}. "
                      ...
                      "orange=right FG, gold=left FG, olive=skin, magenta=pectoralis, "
                      "yellow=clips/calcifications, green=vessels, violet=peritumoural halo. "
                      "Skin, pectoralis, clips, vessels and optional peritumoural halo are excluded; manual pectoralis overrides automatic pectoralis when provided. Gross tumour exclusion is used; PET-auto exclusion is off for speed.")
    ...
    def _export_side_detail_mask(self, side: str, key: str):
        """Return the final per-side detail mask used for export/figure generation."""
        if key == "peritumoral":
            return self._effective_peritumoral_exclusion_for_side(side)
        if key == "pectoralis":
            manual = self._manual_pectoralis_mask(side)
            if manual is not None and np.any(manual):
                return manual.astype(bool)
        ...
        m = d.get(key) if d else None
        return m.astype(bool) if m is not None and np.any(m) else None

    def _training_export_reference_image(self):
        """Pick the best SimpleITK reference image for writing exported NIfTI masks."""
        for attr in ("ct_img_sitk_raw", "ct_image", "pet_img_sitk_raw"):
            ref = getattr(self._app, attr, None)
            if ref is not None and hasattr(ref, "GetSize"):
                return ref
        return None

    def _write_training_nifti(self, out_path: str, arr: np.ndarray, ref_img=None, dtype=np.uint8):
        """Write a numpy array as NIfTI, copying geometry from the reference image when available."""
        data = np.asarray(arr).astype(dtype)
    ...
    def _save_training_qc_png(self, out_path: str):
        """Save a compact QC PNG into the training case folder without opening a dialog."""
        if not BREAST_ML_DEPS_AVAILABLE or not self._fg_masks:
            return False
        # Reuse the same renderer as the normal figure exporter by temporarily calling its internal logic.
        # The method is intentionally conservative: failure is logged but does not block mask export.
        old_dialog = QtWidgets.QFileDialog.getSaveFileName
        try:
            QtWidgets.QFileDialog.getSaveFileName = staticmethod(lambda *args, **kwargs: (out_path, "PNG image (*.png)"))
            self._on_export_figure()
            return os.path.exists(out_path)
        except Exception as exc:
            self._log_msg(f"[Training Export] QC PNG not saved: {exc}")
            return False
        finally:
            QtWidgets.QFileDialog.getSaveFileName = old_dialog

    def _on_export_training_case(self):
        """Save final labelled masks, source volumes, metadata JSON, and QC PNG for future training."""
        ct_hu = getattr(self._app, "ct_hu", None)
        pet_suv = getattr(self._app, "pet_suv", None)
        ref_img = self._training_export_reference_image()
        shape = None
        if ct_hu is not None and hasattr(ct_hu, "shape"):
            shape = ct_hu.shape
        elif pet_suv is not None and hasattr(pet_suv, "shape"):
            shape = pet_suv.shape
        elif ref_img is not None:
            size = ref_img.GetSize()
            shape = (int(size[2]), int(size[1]), int(size[0]))
        if shape is None:
            QtWidgets.QMessageBox.warning(self, "Save Training Case", "No CT/PET volume loaded.")
            return

        right_whole = self._whole_masks.get("right_mask") if self._whole_masks.get("right_mask") is not None else self._roi_right
        left_whole = self._whole_masks.get("left_mask") if self._whole_masks.get("left_mask") is not None else self._roi_left
        right_fg = self._fg_masks.get("right")
        left_fg = self._fg_masks.get("left")
        base_tumor = self._base_tumor_exclusion_mask()
        peri_union = self._effective_peritumoral_exclusion_mask()

        export_masks = {
            "right_breast_mask": right_whole,
            "left_breast_mask": left_whole,
            "right_fg_mask": right_fg,
            "left_fg_mask": left_fg,
            "right_skin_mask": self._export_side_detail_mask("right", "skin"),
            "left_skin_mask": self._export_side_detail_mask("left", "skin"),
            "right_pectoralis_mask": self._export_side_detail_mask("right", "pectoralis"),
            "left_pectoralis_mask": self._export_side_detail_mask("left", "pectoralis"),
            "right_vessels_mask": self._export_side_detail_mask("right", "vessels"),
            "left_vessels_mask": self._export_side_detail_mask("left", "vessels"),
            "right_clips_mask": self._export_side_detail_mask("right", "clips"),
            "left_clips_mask": self._export_side_detail_mask("left", "clips"),
            "tumor_exclusion_mask": base_tumor,
            "peritumoral_exclusion_mask": peri_union,
        }
        if not any(m is not None and np.any(m) for m in export_masks.values()):
            QtWidgets.QMessageBox.information(
                self, "Save Training Case",
                "No labelled masks are available yet.\n\nCreate breast / tumour / pectoralis / FG masks first, then try again."
            )
            return

        root_dir = QtWidgets.QFileDialog.getExistingDirectory(self, "Choose training dataset folder")
        if not root_dir:
            return
        default_case_id = f"case_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        case_id, ok = QtWidgets.QInputDialog.getText(self, "Case ID", "Enter case ID / folder name:", text=default_case_id)
        if not ok:
            return
        case_id = str(case_id).strip() or default_case_id
        safe_case_id = ''.join(ch if (ch.isalnum() or ch in ('-', '_')) else '_' for ch in case_id)
        out_dir = os.path.join(root_dir, safe_case_id)
        os.makedirs(out_dir, exist_ok=True)

        try:
            saved_files = []
            if ct_hu is not None and hasattr(ct_hu, "shape"):
                self._write_training_nifti(os.path.join(out_dir, "ct_hu.nii.gz"), ct_hu, ref_img=ref_img, dtype=np.float32)
                saved_files.append("ct_hu.nii.gz")
            if pet_suv is not None and hasattr(pet_suv, "shape"):
                self._write_training_nifti(os.path.join(out_dir, "pet_suv.nii.gz"), pet_suv, ref_img=ref_img, dtype=np.float32)
                saved_files.append("pet_suv.nii.gz")

            present_masks = {}
            for name, mask in export_masks.items():
                if mask is None or not np.any(mask):
                    continue
                if tuple(mask.shape) != tuple(shape):
                    self._log_msg(f"[Training Export] Skipped {name}: shape {getattr(mask, 'shape', None)} != {shape}")
                    continue
                out_fp = os.path.join(out_dir, f"{name}.nii.gz")
                self._write_training_nifti(out_fp, mask.astype(np.uint8), ref_img=ref_img, dtype=np.uint8)
                saved_files.append(os.path.basename(out_fp))
                present_masks[name] = mask.astype(bool)

            label_map = {
                1: "right_breast_mask", 2: "left_breast_mask",
                3: "right_fg_mask", 4: "left_fg_mask",
                5: "right_skin_mask", 6: "left_skin_mask",
                7: "right_pectoralis_mask", 8: "left_pectoralis_mask",
                9: "right_vessels_mask", 10: "left_vessels_mask",
                11: "right_clips_mask", 12: "left_clips_mask",
                13: "tumor_exclusion_mask", 14: "peritumoral_exclusion_mask",
            }
            multilabel = np.zeros(shape, dtype=np.uint16)
            for lbl in sorted(label_map.keys()):
                name = label_map[lbl]
                m = present_masks.get(name)
                if m is not None and np.any(m):
                    multilabel[m] = np.uint16(lbl)
            if np.any(multilabel):
                self._write_training_nifti(os.path.join(out_dir, "breast_training_multilabel_mask.nii.gz"), multilabel, ref_img=ref_img, dtype=np.uint16)
                saved_files.append("breast_training_multilabel_mask.nii.gz")

            qc_name = "breast_segmentation_qc.png"
            qc_path = os.path.join(out_dir, qc_name)
            if self._save_training_qc_png(qc_path):
                saved_files.append(qc_name)

            meta = {
                "export_type": "breast_training_case",
                "export_version": BREAST_SEGMENTATION_STRICT_VERSION,
                "created_at": datetime.now().isoformat(timespec="seconds"),
                "case_id": safe_case_id,
                "shape_zyx": [int(shape[0]), int(shape[1]), int(shape[2])],
                "spacing_zyx_mm": [float(x) for x in getattr(self._app, "spacing_zyx", (1.0, 1.0, 1.0))],
                "files": sorted(set(saved_files)),
                "mask_voxel_counts": {name: int(np.sum(mask)) for name, mask in present_masks.items()},
                "multilabel_definition": {str(k): v for k, v in label_map.items()},
                "density_results": self._density_results,
                "complexity_results": {k: v for k, v in self._complexity_results.items() if k != "manuscript_shortlist"},
                "has_ct": bool(ct_hu is not None and hasattr(ct_hu, "shape")),
                "has_pet": bool(pet_suv is not None and hasattr(pet_suv, "shape")),
                "notes": "Masks are final user-corrected labels intended for future model training/validation.",
            }
            with open(os.path.join(out_dir, "metadata.json"), "w", encoding="utf-8") as f:
                json.dump(meta, f, indent=2)
            self._log_msg(f"[Training Export] Saved case to {out_dir}")
            QtWidgets.QMessageBox.information(
                self, "Save Training Case",
                f"Training case saved successfully:\n{out_dir}\n\nFiles exported: {len(set(saved_files))}"
            )
        except Exception as exc:
            self._log_msg(f"[Training Export] ERROR: {exc}")
            QtWidgets.QMessageBox.critical(self, "Save Training Case Error", str(exc))
    ...
    def _on_export_figure(self):
        ...
        Peritumoural halo       violet        (160, 32,  240)
        Gross tumor exclusion   red           (220, 30,  30)
        """
        if not BREAST_ML_DEPS_AVAILABLE:
            QtWidgets.QMessageBox.warning(
                self, "Export Figure",
        ...
        left_whole  = _wm.get("left_mask")  if _wm.get("left_mask")  is not None else self._roi_left
        fg_for_display = self._fg_display_masks if isinstance(self._fg_display_masks, dict) and self._fg_display_masks else self._fg_masks
        right_fg    = fg_for_display.get("right")
        left_fg     = fg_for_display.get("left")
        tumor_excl  = self._base_tumor_exclusion_mask()
        ...
        for label, whole_mask in (("RIGHT", right_whole), ("LEFT", left_whole)):
            # If a manual tumour exclusion mask exists on this side, use its
            # centroid as the representative slice so the large exophytic tumour
            # is not missed in the exported panel. Otherwise use the breast ROI.
            side_name = "right" if label == "RIGHT" else "left"
            t_side = None
            if tumor_excl is not None and np.any(tumor_excl):
                side_lateral = _bd_lateral_side_mask(tumor_excl.shape, side_name, gap_frac=0.02)
                t_side = tumor_excl.astype(bool) & side_lateral
            c = _mask_centroid_indices(t_side) if (t_side is not None and np.any(t_side)) else None
            if c is None:
        ...
        try:
            ...
            def _get_ct_slice(orient_axis: int, idx: int) -> np.ndarray:
                ...
                if orient_axis == 0:        # axial: ct[z, :, :]
                    ...
                    out = ct_hu[:, idx, :].astype(float)
                    return out[::-1, :] if _BD_FLIP_BREAST_EXPORT_CORONAL_SAGITTAL else out
            ...
            def _overlay_2d(ax, sl_2d, rgba, pix_aspect=1.0, draw_edge=False):
                ...
                if draw_edge:
                    try:
                        ax.contour(sl_2d.astype(float), levels=[0.5], colors=[rgba[:3]], linewidths=0.7, origin="upper")
                    except Exception:
                        pass
            ...
            layers = [
                # Layer 1: background whole breast
                (right_whole, (0.00, 0.78, 0.78, 0.34), "Whole breast (R)"),
                (left_whole,  (0.27, 0.51, 0.71, 0.34), "Whole breast (L)"),

                # Layer 2: pectoralis muscle
                (_get_side_detail("right", "pectoralis"), (1.00, 0.41, 0.71, 0.88), "Pectoralis muscle"),
                (_get_side_detail("left",  "pectoralis"), (1.00, 0.41, 0.71, 0.88), "Pectoralis muscle"),

                # Layer 3: skin shell
                (_get_side_detail("right", "skin"),       (0.42, 0.56, 0.14, 0.86), "Skin shell"),
                (_get_side_detail("left",  "skin"),       (0.42, 0.56, 0.14, 0.86), "Skin shell"),

                # Layer 4: fibroglandular tissue
                (right_fg,    (1.00, 0.55, 0.00, 0.96), "Fibroglandular (R)"),
                (left_fg,     (0.86, 0.78, 0.00, 0.96), "Fibroglandular (L)"),

                # Layer 5: high-density exclusions
                (_get_side_detail("right", "vessels"),     (0.00, 0.90, 0.45, 0.82), "Blood vessels"),
                (_get_side_detail("left",  "vessels"),     (0.00, 0.90, 0.45, 0.82), "Blood vessels"),
                (_get_side_detail("right", "clips"),       (1.00, 1.00, 0.00, 0.94), "Clips / calcifications"),
                (_get_side_detail("left",  "clips"),       (1.00, 1.00, 0.00, 0.94), "Clips / calcifications"),

                # Layer 6: optional peritumoural exclusion halo
                (_get_side_detail("right", "peritumoral"), (0.63, 0.13, 0.94, 0.70), "Peritumoural exclusion halo"),
                (_get_side_detail("left",  "peritumoral"), (0.63, 0.13, 0.94, 0.70), "Peritumoural exclusion halo"),

                # Layer 7: tumour exclusion zone, absolute top priority
                (tumor_excl,  (1.00, 0.00, 0.00, 0.90), "Tumor exclusion zone"),
            ]
            ...
            for r_idx, (orient_name, orient_axis, ax_letter, sp_mm) in enumerate(orientations):
                ...
                for c_idx, side_label in enumerate(side_labels):
                    ...
                    for mask, rgba, lbl in layers:
                        ...
                        _overlay_2d(ax, sl_2d, rgba, pix_aspect, draw_edge=("Fibroglandular" in lbl))
            ...
            fig.suptitle(f"Breast CT Segmentation — Tissue Layers  [{BREAST_SEGMENTATION_STRICT_VERSION}]\n"
            ...
            fig.savefig(path, dpi=300, bbox_inches="tight",
                        ...
                        facecolor="white")
            plt.close(fig)
            self._log_msg(f"[Figure] Saved to {path} — {BREAST_SEGMENTATION_STRICT_VERSION}")
            QtWidgets.QMessageBox.information(
                self, "Export Figure",
