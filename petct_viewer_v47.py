...
# -*- coding: utf-8 -*-
"""
PET/CT Viewer v47 — Improved breast FG segmentation: skin-distance constraint, breast-local
geometry, two-stage FG flow, QC warnings (based on v21)
---------------------------------------------------------------
...
class BreastSegmentor:
    ...
    # Minimum inward distance (in voxels) from the breast surface before a
    # voxel can be classified as fibroglandular.  Cutaneous / skin tissue
    # sits in the outermost ~4 mm; using 4 voxels at ~1 mm iso-voxels ensures
    # true skin and near-skin fat are excluded from FG candidates.
    _FG_SKIN_EROSION_VOX: int = 4
    ...
    def exclude_non_parenchymal(self, fg_mask: np.ndarray, ct_volume: np.ndarray,
        ...
        """Remove non-parenchymal tissue from a fibroglandular candidate mask.

        All spatial constraints are derived from *breast_mask* when supplied
        (breast-local geometry).  When *breast_mask* is ``None`` an approximate
        breast boundary is reconstructed from HU thresholding (legacy fallback).
        ...
        # ── Derive the reference breast boundary ─────────────────────────────
        if breast_mask is not None and breast_mask.any():
            ref_mask = breast_mask.astype(bool)
        else:
            # Legacy fallback: approximate from HU thresholding
            ny = ct_volume.shape[1]
            ref_mask = (ct_volume >= _BD_HU_BREAST_MIN) & (ct_volume <= _BD_HU_BREAST_MAX)
            y_post_start = max(1, int(ny * self._ANTERIOR_FRACTION))
            ref_mask[:, y_post_start:, :] = False
            ref_mask = _bd_binary_closing(ref_mask.astype(bool), footprint=np.ones((3, 3, 3)))

        # ── 1. Skin-shell exclusion (breast-local, deeper erosion) ───────────
        # Erode the reference breast boundary by _FG_SKIN_EROSION_VOX voxels to
        # create the "inner parenchymal core".  The annular band between the
        # surface and the eroded core is the skin zone: FG cannot be there.
        inner_core = _bd_ndi.binary_erosion(ref_mask, iterations=self._FG_SKIN_EROSION_VOX)
        skin_zone = ref_mask & ~inner_core
        cleaned &= ~skin_zone
        ...
        pectoralis_zone = posterior_region & (ct_volume > _BD_HU_MUSCLE_MIN) & ref_mask
        cleaned &= ~pectoralis_zone

        # ── 3. Metal clips: small bright objects ─────────────────────────────
    ...
    def compute_fg_qc_warnings(self, fg_mask: np.ndarray, breast_mask: np.ndarray,
                                ct_volume: np.ndarray, side: str = "") -> list:
        """Return a list of QC warning strings for the given FG mask.

        Checks
        ------
        * FG voxels in the anterior skin zone exceed 15 % of total FG — indicates
          cutaneous contamination (the failure mode seen in the problem images).
        * FG voxels in the posterior pectoralis zone exceed 20 % of total FG.
        * FG volume is zero when whole-breast volume is non-negligible.
        """
        warnings = []
        ...
        if n_fg == 0:
            if breast_mask.any():
                warnings.append(
                    f"[QC{' '+side.upper() if side else ''}] FG volume is zero — "
                    "no fibroglandular tissue found in breast ROI."
                )
            return warnings

        # Anterior skin zone (outer _FG_SKIN_EROSION_VOX voxels of breast boundary)
        if breast_mask.any():
            inner_core = _bd_ndi.binary_erosion(breast_mask, iterations=self._FG_SKIN_EROSION_VOX)
            skin_zone = breast_mask & ~inner_core
            n_fg_in_skin = int(np.sum(fg_mask & skin_zone))
            skin_pct = 100.0 * n_fg_in_skin / n_fg
            if skin_pct > 15.0:
                warnings.append(
                    f"[QC{' '+side.upper() if side else ''}] ⚠ {skin_pct:.1f}% of FG voxels lie "
                    f"within the skin zone ({n_fg_in_skin}/{n_fg} vox). "
                    "Fibroglandular tissue is never present in cutaneous skin — "
            ...
            if y_coords.size >= 2:
                ...
                n_fg_in_pec = int(np.sum(fg_mask & pec_zone))
                pec_pct = 100.0 * n_fg_in_pec / n_fg
                if pec_pct > 20.0:
                    warnings.append(
                        f"[QC{' '+side.upper() if side else ''}] ⚠ {pec_pct:.1f}% of FG voxels overlap "
                        f"the pectoralis/posterior zone ({n_fg_in_pec}/{n_fg} vox). "
                        "This may indicate mediastinal soft-tissue spillover — "
                        "review posterior breast boundary."
                    )
        return warnings
...
class V21BreastDensityTab(QtWidgets.QWidget):
    ...
    def _on_segment_fg(self):
        ...
        def _compute():
            ...
            for side, roi in (("right", roi_right), ("left", roi_left)):
                ...
                fg_clean = segmentor.exclude_non_parenchymal(fg_raw, ct_snap, breast_mask=roi)
            ...
            # QC warnings per side
            qc_warnings: list = []
            for side, roi in (("right", roi_right), ("left", roi_left)):
                if roi is not None and side in fg_masks:
                    qc_warnings.extend(
                        segmentor.compute_fg_qc_warnings(fg_masks[side], roi, ct_snap, side=side)
                    )
    ...
    def _on_segment_fg_done(self, payload: dict):
        ...
        self._log_msg("[FG] Done. Colour overlays: cyan=right whole, blue=left whole, "
                      ...
                      "Skin (4-vox band), pectoralis and clips are excluded automatically.")
        # Surface QC warnings
        for w in payload.get("qc_warnings", []):
            self._log_msg(w)
    ...
    def _on_compute_density(self):
        ...
        def _compute():
            ...
            for side in ("right", "left"):
                ...
                qc_warnings.extend(
                    segmentor.compute_fg_qc_warnings(fg_clean, breast_mask, ct_snap, side=side)
                )
